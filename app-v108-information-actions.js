...
(() => {
  ...
  function informationPage() {
    ...
    return `<section class="v78-page v107-page v108-information-hub"><header class="v78-page-head v107-page-head"><div><span>Личная база знаний</span><h1>Информация</h1><p>Здесь хранится контекст. Всё, что требует действия, отправляется в планер, задачи или календарь.</p></div><div class="v107-head-actions"><button class="v78-secondary" data-v107-action="open-global-search" type="button">Найти</button><button class="v78-primary" data-v107-action="open-quick-create" type="button">＋ Добавить</button></div></header>
      <button class="v108-info-planner-hero" data-v107-info-route="planner" type="button"><span>${appIcon('planner','coral')}</span><span><small>ПЛАНЫ С ПОНЯТНЫМ РЕЗУЛЬТАТОМ</small><b>Планер</b><p>Обычный план, покупка, событие или проект — без дублирования календаря и задач.</p></span><strong>${activePlans}<small>активных</small></strong><u>Открыть →</u></button>
  ...
  function plannerPage(){
    ...
    return `<section class="v78-page v107-page v108-planner-page">${pageHead('Планы без лишней сложности','Планер','Сначала определите результат, срок и ближайшее действие. Остальные детали добавляются только при необходимости.',`<button class="v78-primary" data-v107-action="edit-plan" type="button">＋ Создать план</button>`)}
  ...
  function editPlan(id='',preset=''){
    const item=id?entityBy('plans',id):{},type=simplePlanType(item.type||preset||'blank'),suggestedTitle={blank:'Новый план',purchase:'Новая покупка или платёж',event:'Новое событие или поездка',project:'Новый проект'}[type]||'Новый план';
    openModal(id?'Редактировать план':`Создать: ${planTypeLabel(type)}`,`<p class="v108-plan-form-note"><b>Минимум для рабочего плана:</b> понятный результат, срок и одно физическое действие.</p><div class="v107-form-grid">${field('Название *','v107_plan_title',item.title||suggestedTitle)}${field('Тип плана','v107_plan_type',options([['blank','Обычный план'],['purchase','Покупка или платёж'],['event','Событие или поездка'],['project','Проект']],type),'select')}${field('Срок','v107_plan_due',item.due,'date')}${field('Статус','v107_plan_status',options([['planned','Запланирован'],['active','В работе'],['paused','На паузе'],['done','Завершён'],['archive','Архив']],item.status||'planned'),'select')}</div>${field('Какой результат должен получиться *','v107_plan_outcome',item.outcome,'textarea','rows="3"')}${field('Следующий конкретный шаг *','v107_plan_next',item.nextAction,'textarea','rows="3"')}<details class="v107-advanced"><summary>Дополнительные детали</summary><div class="v107-form-grid">${field('Приоритет','v107_plan_priority',options([['low','Низкий'],['medium','Средний'],['high','Высокий'],['critical','Критичный']],item.priority||'medium'),'select')}${field('Бюджет, если нужен','v107_plan_budget',item.budget||'','number','min="0" step="0.01"')}${field('Факт вне денежных позиций','v107_plan_actual',item.actualSpend||'','number','min="0" step="0.01"')}${field('Участники','v107_plan_people',list(item.participants).join(', '))}</div>${field('Почему это важно','v107_plan_why',item.why,'textarea')}${field('Документы','v107_plan_documents',list(item.documents).join('\n'),'textarea')}${field('Ссылки','v107_plan_links',list(item.externalLinks).join('\n'),'textarea')}${field('Риски','v107_plan_risks',item.risks,'textarea')}${field('Заметки','v107_plan_notes',item.notes,'textarea')}</details>${formActions('save-plan',id)}`,true);
  }
  function savePlan(id){ const state=ensureData(),old=id?entityBy('plans',id):null,title=val('v107_plan_title'),outcome=val('v107_plan_outcome'),nextAction=val('v107_plan_next'); if(!title)return toast('Введите название');if(!outcome)return toast('Опишите ожидаемый результат');if(!nextAction)return toast('Укажите следующий конкретный шаг'); const type=simplePlanType(val('v107_plan_type')||'blank'); const item=Object.assign({},old||{}, {id:old?.id||uid(),title,type,due:val('v107_plan_due'),status:val('v107_plan_status')||'planned',priority:val('v107_plan_priority')||old?.priority||'medium',budget:num(val('v107_plan_budget')),actualSpend:num(val('v107_plan_actual')),why:val('v107_plan_why'),outcome,nextAction,participants:list(val('v107_plan_people')),documents:list(val('v107_plan_documents')),externalLinks:list(val('v107_plan_links')),risks:val('v107_plan_risks'),notes:val('v107_plan_notes'),blocks:Array.isArray(old?.blocks)?old.blocks:templateBlocks(type),archivedBlocks:Array.isArray(old?.archivedBlocks)?old.archivedBlocks:[],updatedAt:nowIso(),createdAt:old?.createdAt||nowIso()}); state.lifePlans=old?state.lifePlans.map(x=>x.id===id?item:x):[item,...state.lifePlans]; connectPending('plans',item.id); closeModal(); persist('План сохранён'); navigate(`plan-${item.id}`); }

...
(() => {
  ...
  const PLAN_TEMPLATES = [
    ['day','check','mint','День','Фокус, дела, расходы и заметки'],
    ['week','planner','violet','Неделя','Результаты, встречи и бюджет'],
    ['month','planner','blue','Месяц','Цели, платежи и контроль денег'],
    ['purchase','purchases','coral','Покупка','Выбор, бюджет и фиксация факта'],
    ['payment','purchases','amber','Платёж','Срок, сумма и проведение расхода'],
    ['trip','trips','cyan','Поездка','Маршрут, брони, бюджет и документы'],
    ['event','planner','pink','Событие','Участники, задачи и календарь'],
    ['project','target','blue','Проект','Этапы, задачи, риски и результат'],
    ['learning','learning','violet','Обучение','Материалы, практика и вывод'],
    ['blank','plus','slate','Свой шаблон','Добавьте только нужные блоки']
  ];
  ...
  function informationPage() {
    ...
    const groups=[
      {title:'Думать и действовать',text:'Планы, знания и идеи, которые ведут к следующему шагу.',items:[
        ['notes','Заметки','Решения и наблюдения',state.notes.length],['ideas','Идеи','Эксперименты и проекты',state.ideas.length],['learning','Обучение','Материалы и практика',state.learningMaterials.length],['inbox','Входящие','Записи без решения',arr(state,'inbox').length]
      ]},
      {title:'Люди и личная жизнь',text:'Контекст отношений, память, поездки и бережные записи.',items:[
      ...
      {title:'Коллекции и хранение',text:'То, к чему возвращаются: желания, медиа, документы и доступы.',items:[
        ['wishes','Желания','Будущие покупки и мечты',arr(state,'wishes').length],['books','Книги','Чтение, цитаты и выводы',arr(state,'books').length],['films','Фильмы','Просмотр и впечатления',arr(state,'films').length],['documents','Документы','Файлы и важные ссылки',arr(state,'documents').length],['passwords','Пароли','Защищённые доступы',0]
      ]}
    ];
    ...
    const inbox=arr(state,'inbox').length,activePlans=state.lifePlans.filter(plan=>!['done','archive'].includes(plan.status)).length;
    return `<section class="v78-page v107-page v108-information-hub"><header class="v78-page-head v107-page-head"><div><span>Личная база знаний</span><h1>Информация</h1><p>Здесь хранится контекст. Всё, что требует действия, отправляется в планер, задачи или календарь.</p></div><div class="v107-head-actions"><button class="v78-secondary" data-v107-action="open-global-search" type="button">Найти</button><button class="v78-primary" data-v107-action="open-quick-create" type="button">＋ Добавить</button></div></header>
      <button class="v108-info-planner-hero" data-v107-info-route="planner" type="button"><span>${appIcon('planner','coral')}</span><span><small>ЕДИНЫЙ ЦЕНТР ПЛАНИРОВАНИЯ</small><b>Планер всего</b><p>День, неделя, платёж, покупка, поездка, событие или собственный шаблон.</p></span><strong>${activePlans}<small>активных</small></strong><u>Открыть →</u></button>
      <section class="v108-info-groups">${groups.map(group=>`<article class="v108-info-section"><header><h2>${esc(group.title)}</h2><p>${esc(group.text)}</p></header><div>${group.items.map(([route,title,text,count])=>{const [icon,tone]=INFO_FOLDERS[route]||['memory','violet'];return `<button class="v108-info-item" data-v107-info-route="${route}" type="button">${appIcon(icon,tone,'small')}<span><b>${esc(title)}</b><small>${esc(text)}</small></span><em>${count?count:'Пусто'}</em><u>›</u></button>`;}).join('')}</div></article>`).join('')}</section>
      ${smartAssistant(inbox?'Сначала разберите входящие':practice?'Знание пора превратить в практику':stale?'Стоит обновить контекст отношений':'База не требует срочного обслуживания',inbox?`${inbox} ${plural(inbox,'запись ждёт','записи ждут','записей ждут')} решения. Оставьте заметкой, превратите в задачу или удалите лишнее.`:practice?`${practice} ${plural(practice,'материал изучен','материала изучены','материалов изучены')}, но применение пока не зафиксировано.`:stale?`${stale} ${plural(stale,'человек давно без контакта','человека давно без контакта','людей давно без контакта')}.`:'Ничего добавлять ради заполнения не нужно. Сохраняйте только полезный контекст.',[
        inbox?{route:'inbox',label:'Разобрать входящие'}:practice?{route:'learning',label:'Открыть обучение'}:stale?{route:'people',label:'Проверить людей'}:{route:'planner',label:'Открыть планер'}
      ])}
    </section>`;
  }
  ...
  function plannerPage(){
    ...
    const active=items.filter(item=>!['done','archive'].includes(item.status)),moneyItems=active.flatMap(plan=>purchaseItems(plan).map(item=>({plan,item}))).filter(({item})=>item.status!=='cancelled');
    const planned=moneyItems.reduce((sum,{item})=>sum+num(item.plannedBudget??item.amount),0),actual=moneyItems.filter(({item})=>moneyItemDone(item)).reduce((sum,{item})=>sum+(num(item.actualAmount)||num(item.plannedBudget??item.amount)),0),recorded=moneyItems.filter(({item})=>moneyItemDone(item)).length;
    const overdue=active.filter(item=>item.due&&item.due<today()).length;
    return `<section class="v78-page v107-page v108-planner-page">${pageHead('Единый центр действий','Планер всего','Один шаблон собирает задачи, сроки, платежи, покупки, документы и итог — без дублирования по разделам.',`<button class="v78-primary" data-v107-action="edit-plan" type="button">＋ Свой план</button>`)}
      <section class="v108-planner-kpis"><article><small>Активные планы</small><b>${active.length}</b><span>${overdue?`${overdue} просрочено`:'сроки под контролем'}</span></article><article><small>Запланировано</small><b>${money(planned)}</b><span>${moneyItems.length} денежных позиций</span></article><article><small>Фактически</small><b>${money(actual)}</b><span>${recorded} учтено</span></article><article><small>Осталось по плану</small><b>${money(Math.max(0,planned-actual))}</b><span>план минус факт</span></article></section>
      <section class="v108-template-panel"><header><div><small>БЫСТРЫЙ СТАРТ</small><h2>Выберите готовый шаблон</h2><p>Любой шаблон можно изменить: скрыть лишние блоки и добавить свои.</p></div></header><div>${PLAN_TEMPLATES.map(([type,icon,tone,title,text])=>`<button data-v107-action="create-plan-template" data-template="${type}" type="button">${appIcon(icon,tone,'small')}<span><b>${esc(title)}</b><small>${esc(text)}</small></span><u>＋</u></button>`).join('')}</div></section>
      ...
      <section class="v108-plan-list v107-filter-grid">${items.map(plannerCard).join('')||empty('Создайте план из готового шаблона: от одного дня до проекта, поездки или покупки.')}</section>
      ${smartAssistant(overdue?'Есть планы с прошедшим сроком':planned&&actual>planned?'Фактические расходы выше плана':active.length?'Следующий шаг важнее количества блоков':'Начните с ближайшего результата',overdue?`У ${overdue} ${plural(overdue,'плана прошёл','планов прошёл','планов прошёл')} срок. Перенесите дату, завершите или отправьте в архив.`:planned&&actual>planned?`Перерасход составляет ${money(actual-planned)}. Проверьте фактические суммы покупок.`:active.length?'Откройте один активный план и убедитесь, что указан ближайший физический шаг.':'Выберите шаблон дня, недели, покупки или проекта — все поля можно заполнять постепенно.',[{action:'edit-plan',label:'Создать свой план'}])}
    </section>`;
  }

  function plannerCard(plan){
    const totals=purchaseTotals(plan),items=(plan.blocks||[]).filter(block=>block.type!=='purchases').flatMap(block=>Array.isArray(block.items)?block.items:[]).filter(item=>item&&'done'in item),done=items.filter(item=>item.done).length,progress=items.length?Math.round(done/items.length*100):(plan.status==='done'?100:0),budget=num(plan.budget)||totals.planned,actual=Math.max(num(plan.actualSpend),totals.actual);
    return `<article class="v108-plan-card" data-v107-filter-card data-status="${esc(plan.status||'planned')}"><button data-v107-info-route="plan-${plan.id}" type="button"><header>${appIcon('planner',plan.type==='purchase'?'coral':plan.type==='trip'?'cyan':'violet','small')}<span><small>${esc(planTypeLabel(plan.type))} · ${esc(planStatusLabel(plan.status))}</small><h2>${esc(plan.title||'Без названия')}</h2></span><u>›</u></header><p>${esc(plan.nextAction||plan.outcome||plan.why||'Добавьте следующий шаг.')}</p><div class="v108-plan-card-stats"><span><small>Срок</small><b>${plan.due?formatShort(plan.due):'Без даты'}</b></span><span><small>План</small><b>${money(budget)}</b></span><span><small>Факт</small><b>${money(actual)}</b></span><span><small>Готово</small><b>${progress}%</b></span></div><i><b style="width:${Math.min(100,progress)}%"></b></i></button></article>`;
  }
  ...
  function planDetail(id){
    ...
    const totals=purchaseTotals(plan),budget=num(plan.budget)||totals.planned,actual=Math.max(num(plan.actualSpend),totals.actual);
    return `<section class="v78-page v107-page"><header class="v107-detail-head"><div><button class="v78-back" data-v107-info-route="planner">← Планер</button><small>${planTypeLabel(plan.type)} · ${planStatusLabel(plan.status)}</small><h1>${esc(plan.title)}</h1><p>${esc(plan.why||plan.outcome||'')}</p></div><div class="v107-head-actions"><button class="v78-secondary" data-v107-action="edit-plan" data-id="${id}">Редактировать</button><button class="v78-primary" data-v107-action="add-plan-block" data-id="${id}">＋ Добавить блок</button></div></header>
      <section class="v107-plan-summary"><article><small>Срок</small><b>${formatDate(plan.due)}</b></article><article><small>Бюджет плана</small><b>${money(budget)}</b></article><article><small>Факт по плану</small><b>${money(actual)}</b></article><article><small>Выполнено</small><b>${progress}%</b></article><article class="wide"><small>Следующий шаг</small><b>${esc(plan.nextAction||'Не определён')}</b></article></section>
      ${purchaseBoard(plan)}
  ...
  function planTypeLabel(value){return ({blank:'Свой шаблон',day:'План дня',week:'План недели',month:'План месяца',purchase:'Покупка',payment:'Платёж',trip:'Поездка',event:'Событие',gift:'Подарок',repair:'Ремонт',move:'Переезд',health:'Здоровье',relationship:'Отношения',learning:'Обучение',project:'Личный проект',finance:'Финансовый план',important:'Важное мероприятие'})[value]||'План';}
  ...
  function editPlan(id='',preset=''){
    const item=id?entityBy('plans',id):{},type=item.type||preset||'blank',suggestedTitle={day:'План дня',week:'План недели',month:'План месяца',purchase:'Новая покупка',payment:'Запланированный платёж',trip:'Новая поездка',event:'Новое событие',project:'Новый проект',learning:'План обучения'}[type]||'';
    openModal(id?'Редактировать план':`Создать: ${planTypeLabel(type)}`,`<div class="v107-form-grid">${field('Название *','v107_plan_title',item.title||suggestedTitle)}${field('Шаблон','v107_plan_type',options([['day','День'],['week','Неделя'],['month','Месяц'],['purchase','Покупка'],['payment','Платёж'],['trip','Поездка'],['event','Событие'],['gift','Подарок'],['repair','Ремонт'],['move','Переезд'],['health','Здоровье'],['relationship','Отношения'],['learning','Обучение'],['project','Личный проект'],['finance','Финансовый план'],['important','Важное мероприятие'],['blank','Свой шаблон']],type),'select')}${field('Срок','v107_plan_due',item.due||((type==='day')?today():''),'date')}${field('Статус','v107_plan_status',options([['planned','Запланирован'],['active','В работе'],['paused','На паузе'],['done','Завершён'],['archive','Архив']],item.status||'planned'),'select')}${field('Приоритет','v107_plan_priority',options([['low','Низкий'],['medium','Средний'],['high','Высокий'],['critical','Критичный']],item.priority||'medium'),'select')}${field('Общий бюджет','v107_plan_budget',item.budget||'','number','min="0" step="0.01"')}${field('Факт вне списка покупок','v107_plan_actual',item.actualSpend||'','number','min="0" step="0.01"')}</div><p class="v108-plan-form-note">Расходы и покупки внутри плана суммируются автоматически. Поле факта нужно только для сумм, которые не внесены отдельными позициями.</p>${field('Зачем это нужно','v107_plan_why',item.why,'textarea')}${field('Желаемый результат','v107_plan_outcome',item.outcome,'textarea')}${field('Следующий шаг','v107_plan_next',item.nextAction,'textarea')}${field('Участники','v107_plan_people',list(item.participants).join(', '))}${field('Документы','v107_plan_documents',list(item.documents).join('\n'),'textarea')}${field('Ссылки','v107_plan_links',list(item.externalLinks).join('\n'),'textarea')}${field('Риски','v107_plan_risks',item.risks,'textarea')}${field('Заметки','v107_plan_notes',item.notes,'textarea')}${formActions('save-plan',id)}`,true);
  }
  ...
  function templateBlocks(type){
    const map={day:['tasks','calendar','purchases','budget','notes','outcome'],week:['tasks','calendar','purchases','budget','notes','outcome'],month:['tasks','calendar','purchases','budget','risks','notes','outcome'],purchase:['checklist','options','comparison','purchases','budget','documents','outcome'],payment:['purchases','calendar','documents','notes','outcome'],trip:['checklist','budget','participants','calendar','bookings','route','documents','purchases','outcome'],event:['checklist','tasks','budget','participants','calendar','documents','outcome'],gift:['options','comparison','budget','purchases','checklist','outcome'],repair:['checklist','tasks','purchases','budget','risks','documents','outcome'],move:['checklist','tasks','purchases','budget','calendar','documents','outcome'],health:['tasks','calendar','documents','notes','outcome'],relationship:['tasks','calendar','participants','notes','outcome'],learning:['checklist','tasks','calendar','notes','outcome'],project:['tasks','checklist','budget','purchases','risks','notes','outcome'],finance:['budget','purchases','calendar','risks','notes','outcome'],important:['checklist','tasks','participants','calendar','documents','risks','outcome'],blank:[]};

...
/* ===== Folder Drilldown + Debt Date Final — чистая иерархия папок ===== */
(function(){
  const BUILD='folder-drilldown-debt-date-final-20260630';
  try{localStorage.setItem('secondBrainOS.currentBuild', BUILD);}catch(e){}
  function q(v){try{return typeof esc==='function'?esc(v):String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}catch(e){return String(v??'')}}
  function num2(v){try{return typeof num==='function'?num(v):Number(String(v??0).replace(/[^0-9,.-]/g,'').replace(',','.'))||0}catch(e){return 0}}
  function rub(v){try{return typeof money==='function'?money(v):`${Math.round(num2(v)).toLocaleString('ru-RU')} ₽`}catch(e){return `${Math.round(num2(v)).toLocaleString('ru-RU')} ₽`}}
  function uid2(){try{return uid()}catch(e){return 'id_'+Date.now().toString(36)+Math.random().toString(36).slice(2)}}
  function today2(){try{return todayKey()}catch(e){return new Date().toISOString().slice(0,10)}}
  function month2(){try{return (state.settings&&state.settings.currentMonth)||monthKey()}catch(e){return new Date().toISOString().slice(0,7)}}
  function arr(k){if(!state[k]||!Array.isArray(state[k]))state[k]=[];return state[k]}
  function save2(silent){try{save(silent!==false)}catch(e){try{localStorage.setItem(STORE_KEY,JSON.stringify({state}))}catch(_){}}}
  function titleMonth2(m){try{return new Date(String(m||month2())+'-01T12:00:00').toLocaleDateString('ru-RU',{month:'long',year:'numeric'})}catch(e){return m||month2()}}
  function empty2(t){try{return empty(t)}catch(e){return `<div class="mini-box muted">${q(t)}</div>`}}
  function toast2(t){try{toast(t)}catch(e){console.log('[SBOS]',t)}}
  function ensureDrill(){
    state.settings=state.settings||{};
    if(!state.settings.folderDrill)state.settings.folderDrill={};
    ['finance','budget','goals','planning','spheres','habits','debts','trips','import','diagnostics'].forEach(k=>{if(!state.settings.folderDrill[k])state.settings.folderDrill[k]={folder:'',sub:''};});
    ['tasks','calendarEvents','plannedPurchases','goals','habits','notes','people','books','ideas','media','files','trips','debts','operations','subconsciousDiary'].forEach(arr);
    state.settings.hiddenDebtIds=Array.isArray(state.settings.hiddenDebtIds)?state.settings.hiddenDebtIds:[];
  }
  function setDrill(section, folder, sub){ensureDrill();state.settings.folderDrill[section]={folder:folder||'',sub:sub||''};save2(true);render();}
  function backDrill(section){ensureDrill();const d=state.settings.folderDrill[section]||{}; if(d.sub) setDrill(section,d.folder,''); else setDrill(section,'','');}
  function getDrill(section){ensureDrill();return state.settings.folderDrill[section]||{folder:'',sub:''}}
  function card(section, folder, title, ico, count, desc, sub){
    return `<button class="folder-card drill-card" data-action="setDrillFolder" data-section="${q(section)}" data-folder="${q(folder)}" data-sub="${q(sub||'')}"><div class="folder-ico">${q(ico)}</div><strong>${q(title)}</strong><small>${q(count||'')}</small>${desc?`<p>${q(desc)}</p>`:''}<span class="pill blue">Открыть</span></button>`;
  }
  function rootPage(section,title,subtitle,cardsHtml,extra){
    return layout(title,subtitle,`<section class="card drill-home"><div class="card-head"><div><h3>Подпапки</h3><p class="muted small">Открывай только нужную категорию — без длинной перегруженной страницы.</p></div><span class="pill blue">папка → подпапки</span></div><div class="folder-list drill-grid">${cardsHtml}</div></section>${extra||''}`);
  }
  function subHeader(section,title,subtitle){return `<section class="card drill-head"><div><button class="mini-btn" data-action="backDrill" data-section="${q(section)}">← Назад</button><h3>${q(title)}</h3><p class="muted small">${q(subtitle||'Отдельный лист подпапки. На экране только нужные записи.')}</p></div></section>`}
  function rowSimple(label,title,sub,act,id){return `<div class="list-row clean-folder-row"><span class="pill blue">${q(label)}</span><div><div class="row-title">${q(title)}</div><div class="row-sub">${q(sub||'')}</div></div>${act?`<div class="button-row"><button class="mini-btn" data-action="${q(act)}" data-id="${q(id)}">Ред.</button></div>`:''}</div>`}
  function panelList(title,rows,add){return `<section class="card" style="margin-top:18px"><div class="card-head"><h3>${q(title)}</h3>${add?`<button class="btn secondary" data-action="${q(add)}">Добавить</button>`:''}</div><div class="note-list">${rows.join('')||empty2('Пока пусто')}</div></section>`}

  function kindName(v){v=String(v||'').trim(); if(/виш|wish|хотел/i.test(v))return 'Вишлист'; if(/плат|обяз/i.test(v))return 'Платёж'; if(/будущ/i.test(v))return 'Будущая трата'; if(/покуп/i.test(v))return 'Покупка'; return v||'Покупка'}
  function planItems(){
    ensureDrill(); const out=[];
    arr('tasks').forEach(t=>{if(String(t.status||'')==='Готово')return; out.push({src:'task',id:t.id,title:t.title||'Задача',kind:'Задача',owner:t.owner||'Я',amount:0,month:String(t.due||month2()).slice(0,7)||month2(),date:t.due||'',time:t.time||'',meta:t.area||'Личное',image:t.image||'',link:''})});
    arr('calendarEvents').forEach(e=>out.push({src:'event',id:e.id,title:e.title||'Событие',kind:'Календарь',owner:e.owner||'Общее',amount:0,month:String(e.date||month2()).slice(0,7)||month2(),date:e.date||'',time:e.time||'',meta:e.area||'Календарь',image:e.image||'',link:''}));
    arr('plannedPurchases').forEach(p=>out.push({src:'purchase',id:p.id,title:p.title||'Пункт',kind:kindName(p.kind),owner:p.owner||'Я',amount:num2(p.amount),month:p.month||month2(),date:p.due||'',time:'',meta:`${p.category||'Без категории'} · ${p.includeInBudget!==false?'в прогнозе':'вне прогноза'}`,image:p.image||'',link:p.link||'',include:p.includeInBudget!==false&&p.active!==false}));
    return out;
  }
  const planFolders=[['tasks','Задачи','✓','дела и фокус'],['calendar','Календарь','◷','события и даты'],['wishlist','Вишлист','✦','желания: Я, Полина, Общее'],['payments','Платежи','₽','регулярные обязательства'],['future','Будущие траты','↗','предстоящие расходы'],['purchases','Покупки','◇','покупки и картинки'],['all','Все пункты','☷','общая лента']];
  function itemsByPlanFolder(f){const it=planItems(); if(f==='tasks')return it.filter(x=>x.kind==='Задача'); if(f==='calendar')return it.filter(x=>x.kind==='Календарь'); if(f==='wishlist')return it.filter(x=>x.kind==='Вишлист'); if(f==='payments')return it.filter(x=>x.kind==='Платёж'); if(f==='future')return it.filter(x=>x.kind==='Будущая трата'); if(f==='purchases')return it.filter(x=>x.kind==='Покупка'); return it;}
  function rowPlan(x){const img=x.image?`<img class="drill-img" src="${q(x.image)}" loading="lazy" onerror="this.remove()">`:''; const act=x.src==='task'?'editTask':x.src==='event'?'editEvent':'editPurchase'; const del=x.src==='purchase'?`<button class="mini-btn red" data-action="deletePurchase" data-id="${q(x.id)}">Удалить</button>`:''; const link=x.link?`<a class="mini-btn blue" href="${q(x.link)}" target="_blank" rel="noopener">Ссылка</a>`:''; return `<div class="drill-row ${q(x.kind.toLowerCase())}">${img}<div class="drill-row-main"><div class="button-row compact-row"><span class="pill blue">${q(x.kind)}</span><span class="pill ${x.owner==='Полина'?'violet':x.owner==='Общее'?'green':'blue'}">${q(x.owner)}</span>${x.amount?`<span class="pill green">${rub(x.amount)}</span>`:''}</div><div class="row-title">${q(x.title)}</div><div class="row-sub">${q(x.date||titleMonth2(x.month))}${x.time?' · '+q(x.time):''} · ${q(x.meta||'')}</div></div><div class="button-row"><button class="mini-btn" data-action="${q(act)}" data-id="${q(x.id)}">Ред.</button>${del}${link}</div></div>`}
  function planningRoot(){const m=month2(); const cards=planFolders.map(f=>{const rows=itemsByPlanFolder(f[0]); const sum=rows.filter(x=>x.include!==false).reduce((a,b)=>a+num2(b.amount),0); return card('planning',f[0],f[1],f[2],`${rows.length} пунктов${sum?' · '+rub(sum):''}`,f[3]);}).join(''); const extra=`<section class="grid cols-3" style="margin-top:18px"><article class="card"><h3>Запланировано за ${q(titleMonth2(m))}</h3><div class="value sm blue">${rub(planItems().filter(x=>x.month===m&&x.include!==false).reduce((a,b)=>a+num2(b.amount),0))}</div></article><article class="card"><h3>Дела</h3><div class="value sm">${itemsByPlanFolder('tasks').length+itemsByPlanFolder('calendar').length}</div></article><article class="card"><h3>Фильтр</h3><p class="muted small">Внутри подпапок можно редактировать каждый пункт.</p></article></section>`; return rootPage('planning','Планирование','Единая папка: дела, календарь, вишлист, платежи, будущие траты и покупки.',cards,extra)}
  function planningSub(f){const meta=planFolders.find(x=>x[0]===f)||planFolders[0]; let add=f==='tasks'?'addTask':f==='calendar'?'addEvent':f==='wishlist'?'addWishlistItem':f==='payments'?'addPaymentItem':'addPurchase'; const rows=itemsByPlanFolder(f).map(rowPlan); return layout('Планирование · '+meta[1],meta[3],subHeader('planning',meta[1],'Это отдельный лист подпапки планирования.')+`<section class="card" style="margin-top:18px"><div class="card-head"><h3>${q(meta[1])}</h3><button class="btn secondary" data-action="${q(add)}">Добавить</button></div><div class="note-list">${rows.join('')||empty2('В этой подпапке пока пусто')}</div></section>`)}
  function planningPageFinal(){const d=getDrill('planning');return d.folder?planningSub(d.folder):planningRoot()}

  function lifeFoldersFinal(){return [
    ['people','Люди','◌',arr('people').length,'контакты, даты, подарки'],['notes','Заметки','✎',arr('notes').length,'мысли, рефлексии, записи'],['planning','Планирование','☷',planItems().length,'дела и будущие расходы'],['habits','Привычки','◷',arr('habits').length,'трекер и ежедневное интервью'],['books','Книги','▣',arr('books').length,'чтение, цитаты, выводы'],['ideas','Идеи','✦',arr('ideas').length,'замыслы и гипотезы'],['personal','Личная жизнь','♡',arr('media').length,'фильмы, сериалы, личное'],['documents','Документы','▤',arr('files').length,'файлы и ссылки'],['trips','Путешествия','✈',arr('trips').length,'маршруты и бюджеты'],['diary','Дневник','◉',arr('subconsciousDiary').length,'интервью с подсознанием'],['wishlist','Вишлист','✦',itemsByPlanFolder('wishlist').length,'желания с пометкой кому'],['payments','Платежи','₽',itemsByPlanFolder('payments').length,'обязательные платежи']
  ]}
  const lifeSubs={
    people:[['all','Все люди','контакты'],['birthdays','Дни рождения','даты'],['gifts','Подарки','идеи подарков'],['talks','Темы разговора','что обсудить']],
    notes:[['all','Все заметки','все записи'],['reflection','Рефлексия','мысли дня'],['diary','Дневник','интервью'],['ideas','Идеи','заметки-идеи']],
    planning:planFolders.map(f=>[f[0],f[1],f[3]]),
    habits:[['interview','Интервью с подсознанием','каждый день'],['tracker','Трекер привычек','отметки'],['month','Месяц','календарь']],
    books:[['all','Все книги','список'],['quotes','Цитаты','что зацепило'],['insights','Выводы','что применить']],
    ideas:[['all','Все идеи','список'],['active','В работе','активные'],['archive','Архив','позже']],
    personal:[['watch','Что посмотреть','фильмы/сериалы'],['notes','Личное','заметки']],
    documents:[['all','Все документы','файлы'],['links','Ссылки','URL'],['important','Важное','приоритет']],
    trips:[['all','Все поездки','список'],['budget','Бюджет','расходы'],['future','Будущие','планы']],
    diary:[['today','Сегодня','начать интервью'],['all','Все записи','история'],['reflection','Рефлексия','цитаты и выводы']],
    wishlist:[['all','Весь вишлист','все'],['Я','Мой','личное'],['Полина','Полины','для Полины'],['Общее','Общее','для двоих']],
    payments:[['all','Все платежи','все'],['Я','Мои','личные'],['Полина','Полины','для Полины'],['Общее','Общие','для двоих']]
  };
  function spheresRoot(){const cards=lifeFoldersFinal().map(f=>card('spheres',f[0],f[1],f[2],`${f[3]} записей`,f[4])).join(''); return rootPage('spheres','Сферы жизни','Сначала основные папки. Внутри каждой — свои подпапки на отдельном листе.',cards)}
  function spheresFolder(folder){const f=lifeFoldersFinal().find(x=>x[0]===folder)||lifeFoldersFinal()[0]; const subs=lifeSubs[folder]||[['all','Все записи','содержимое']]; const cards=subs.map(s=>card('spheres',folder,s[1],s[0]==='all'?'☷':'•',s[2]||'',`подпапка ${f[1]}`,s[0])).join(''); return layout('Сферы жизни · '+f[1],f[4],subHeader('spheres',f[1],'Внутри основной папки лежат подпапки. Выбери одну.')+`<section class="card drill-home" style="margin-top:18px"><div class="folder-list drill-grid">${cards}</div></section>`)}
  function rowsByNotes(filter){let list=arr('notes'); if(filter==='reflection')list=list.filter(x=>(x.tags||[]).includes('рефлексия')); if(filter==='diary')list=list.filter(x=>/подсознан|интервью|дневник/i.test([x.title,x.folder,(x.tags||[]).join(' ')].join(' '))); if(filter==='ideas')list=list.filter(x=>/иде/i.test([x.title,x.folder,(x.tags||[]).join(' ')].join(' '))); return list.map(x=>rowSimple((x.tags||[])[0]||'Заметка',x.title||'Заметка',String(x.text||'').slice(0,160),'editNote',x.id))}
  function renderWishlist(owner){let rows=itemsByPlanFolder('wishlist'); if(owner&&owner!=='all')rows=rows.filter(x=>x.owner===owner); return panelList(owner&&owner!=='all'?'Вишлист · '+owner:'Весь вишлист',rows.map(rowPlan),'addWishlistItem')}
  function renderPayments(owner){let rows=itemsByPlanFolder('payments'); if(owner&&owner!=='all')rows=rows.filter(x=>x.owner===owner); return panelList(owner&&owner!=='all'?'Платежи · '+owner:'Все платежи',rows.map(rowPlan),'addPaymentItem')}
  function dailyInterviewPanel(){
    const h=arr('habits').find(x=>/интервью с подсознанием/i.test(x.title||'')); const done=!!(h&&h.marks&&h.marks[today2()]);
    const recent=arr('subconsciousDiary').slice(0,5).map(e=>rowSimple('Дневник',e.date||'',(e.truth||e.step||e.notebook||'Запись').slice(0,150),'editDailySubconsciousInterview',e.id));
    return `<section class="card notebook-card" style="margin-top:18px"><div class="card-head"><div><h3>Ежедневное интервью с подсознанием</h3><p class="muted small">Заполняй каждый день. Запись сохранится в дневник и в заметки.</p></div><span class="pill ${done?'green':'violet'}">${done?'сегодня готово':'ждёт ответа'}</span></div><div class="notebook-quote"><span>Цитата</span><b>“${q((typeof dailyQuote==='function'?dailyQuote().quote:'Честность с собой — начало порядка.'))}”</b></div><div class="notebook-question"><span>Вопрос дня</span><b>${q(typeof questionOfDay==='function'?questionOfDay():'Что я уже знаю, но пока не хочу признать?')}</b></div><div class="actions"><button class="btn" data-action="openDailySubconsciousInterviewFinal">Начать интервью</button></div></section>${panelList('Последние записи',recent,'')}`
  }
  function renderSpheresSub(folder,sub){
    let title=(lifeSubs[folder]||[]).find(x=>x[0]===sub)?.[1]||'Все'; let body='';
    if(folder==='planning') body=planningSub(sub||'all').replace(/^<div class="page">|<\/div>$/g,'');
    else if(folder==='wishlist') body=renderWishlist(sub||'all');
    else if(folder==='payments') body=renderPayments(sub||'all');
    else if(folder==='diary') body=(sub==='today')?dailyInterviewPanel():panelList('Дневник',arr('subconsciousDiary').map(e=>rowSimple('Дневник',e.date||'',(e.truth||e.step||e.notebook||'').slice(0,160),'editDailySubconsciousInterview',e.id)),'');
    else if(folder==='habits') body=(sub==='interview')?dailyInterviewPanel():(typeof habitsFolder==='function'?habitsFolder():panelList('Привычки',arr('habits').map(h=>rowSimple('Привычка',h.title||'',h.note||'','editHabit',h.id)),'addHabit'));
    else if(folder==='people'){let l=arr('people'); if(sub==='birthdays')l=l.filter(p=>p.birthday); if(sub==='gifts')l=l.filter(p=>p.gifts); if(sub==='talks')l=l.filter(p=>p.talkIdeas); body=panelList(title,l.map(p=>rowSimple('Человек',p.name||'',p.birthday||p.gifts||p.talkIdeas||'','editPerson',p.id)),'addPerson')}
    else if(folder==='notes') body=panelList(title,rowsByNotes(sub),'addNote');
    else if(folder==='books'){let l=arr('books'); if(sub==='quotes'){const r=l.flatMap(b=>(b.quotes||[]).map(x=>rowSimple('Цитата',b.title||'',x,'editBook',b.id))); body=panelList(title,r,'addBook')} else if(sub==='insights'){body=panelList(title,l.filter(b=>b.insight).map(b=>rowSimple('Вывод',b.title||'',b.insight,'editBook',b.id)),'addBook')} else body=panelList(title,l.map(b=>rowSimple('Книга',b.title||'',b.insight||((b.quotes||[])[0])||'','editBook',b.id)),'addBook')}
    else if(folder==='ideas') body=panelList(title,arr('ideas').map(i=>rowSimple('Идея',i.title||'',i.note||'','editIdea',i.id)),'addIdea');
    else if(folder==='personal') body=typeof personalFolder==='function'?personalFolder():panelList(title,arr('media').map(m=>rowSimple('Личное',m.title||'',m.note||'','editMedia',m.id)),'addMedia');
    else if(folder==='documents') body=typeof documentsFolder==='function'?documentsFolder():panelList(title,arr('files').map(f=>rowSimple('Документ',f.title||'',f.url||f.note||'','editFile',f.id)),'addFileNote');
    else if(folder==='trips') body=typeof tripsPanel==='function'?tripsPanel():panelList(title,arr('trips').map(t=>rowSimple('Поездка',t.title||'',t.budget?rub(t.budget):'','editTrip',t.id)),'addTrip');
    return layout('Сферы жизни · '+title,'Отдельный лист подпапки.',subHeader('spheres',title,'Только выбранная категория, без общей перегрузки.')+body)
  }
  function spheresPageFinal(){const d=getDrill('spheres'); if(!d.folder)return spheresRoot(); if(!d.sub)return spheresFolder(d.folder); return renderSpheresSub(d.folder,d.sub)}

  function financePageFinal(){const d=getDrill('finance'); const cards=[['summary','Сводка','◔','остаток и прогноз'],['operations','Операции','≡',`${arr('operations').length} записей`],['categories','Категории','▦',`${arr('categories').length} категорий`],['planned','План по месяцам','☷',`${rub(planItems().filter(x=>x.include!==false&&x.amount).reduce((a,b)=>a+num2(b.amount),0))}`],['debts','Долги','▣',`${rub((typeof activeDebts==='function'?activeDebts():arr('debts')).reduce((a,b)=>a+num2(b.amount),0))}`]].map(f=>card('finance',f[0],f[1],f[2],f[3],'')); if(!d.folder)return rootPage('finance','Финансы','Финансы разбиты на подпапки: сводка, операции, категории, план и долги.',cards.join('')); let body=''; if(d.folder==='operations')body=panelList('Операции',arr('operations').slice(0,80).map(o=>typeof rowOperation==='function'?rowOperation(o):rowSimple(o.type,o.category,o.amount,'editOperation',o.id)),'addExpense'); else if(d.folder==='categories')body=panelList('Категории',arr('categories').map(c=>rowSimple(c.type==='income'?'Доход':'Расход',c.name,c.limit?rub(c.limit):'лимит —','editCategory',c.id)),'addCategory'); else if(d.folder==='planned')body=typeof monthlyForecastBlock==='function'?monthlyForecastBlock():panelList('План',planItems().filter(x=>x.amount).map(rowPlan),'addPurchase'); else if(d.folder==='debts')body=debtsSubContent('all'); else body=(typeof financePage==='function'&&financePage!==financePageFinal?financePage():panelList('Сводка',[],'')); return layout('Финансы · '+(d.folder||''),'Отдельный лист финансовой подпапки.',subHeader('finance','Финансы','Назад к подпапкам')+body)}
  function budgetPageFinal(){const d=getDrill('budget'); const cards=[['limits','Лимиты','◉',`${arr('categories').filter(c=>c.limit).length} лимитов`],['planned','Запланировано','☷',rub(planItems().filter(x=>x.include!==false&&x.amount&&x.month===month2()).reduce((a,b)=>a+num2(b.amount),0))],['wishlist','Вишлист','✦',`${itemsByPlanFolder('wishlist').length} пунктов`],['payments','Платежи','₽',`${itemsByPlanFolder('payments').length} пунктов`],['purchases','Покупки','◇',`${itemsByPlanFolder('purchases').length} пунктов`]].map(f=>card('budget',f[0],f[1],f[2],f[3],'')); if(!d.folder)return rootPage('budget','Бюджет','Лимиты, плановые расходы и регулярные платежи разнесены по подпапкам.',cards.join('')); let body=d.folder==='limits'?(typeof budgetPage==='function'&&budgetPage!==budgetPageFinal?budgetPage():panelList('Лимиты',[],'')):d.folder==='wishlist'?renderWishlist('all'):d.folder==='payments'?renderPayments('all'):panelList('Запланированные расходы',planItems().filter(x=>x.amount&&x.include!==false&&(d.folder==='planned'||x.kindName===d.folder)).map(rowPlan),'addPurchase'); return layout('Бюджет','Отдельный лист бюджета.',subHeader('budget','Бюджет','Назад к подпапкам')+body)}
  function goalsPageFinal(){const d=getDrill('goals'); const gs=arr('goals'); const cards=[['active','Активные','◎',`${gs.filter(g=>g.status!=='Готово').length} целей`],['images','С картинками','▧',`${gs.filter(g=>g.image).length} целей`],['completed','Завершённые','✓',`${gs.filter(g=>g.status==='Готово').length} целей`]].map(f=>card('goals',f[0],f[1],f[2],f[3],'')); if(!d.folder)return rootPage('goals','Цели SMART','Цели разложены по подпапкам.',cards.join(''),`<div class="actions"><button class="btn" data-action="addGoal">Добавить цель</button></div>`); let l=gs; if(d.folder==='active')l=gs.filter(g=>g.status!=='Готово'); if(d.folder==='images')l=gs.filter(g=>g.image); if(d.folder==='completed')l=gs.filter(g=>g.status==='Готово'); return layout('Цели SMART','Отдельная подпапка целей.',subHeader('goals','Цели SMART','Назад к подпапкам')+panelList('Цели',l.map(g=>rowSimple('SMART',g.title||'',`${g.progress||0}% · ${g.image?'есть картинка':'без картинки'}`,'editGoal',g.id)),'addGoal'))}
  function habitsPageFinal(){const d=getDrill('habits'); const cards=[['interview','Интервью с подсознанием','◉','ежедневно'],['tracker','Трекер привычек','◷',`${arr('habits').length} привычек`],['month','Месяц','▦','календарь отметок']].map(f=>card('habits',f[0],f[1],f[2],f[3],'')); if(!d.folder)return rootPage('habits','Привычки','Ежедневные ритуалы и интервью — в отдельных подпапках.',cards.join('')); const body=d.folder==='interview'?dailyInterviewPanel():(typeof habitsFolder==='function'?habitsFolder():panelList('Привычки',arr('habits').map(h=>rowSimple('Привычка',h.title||'',h.note||'','editHabit',h.id)),'addHabit')); return layout('Привычки','Отдельная подпапка.',subHeader('habits','Привычки','Назад к подпапкам')+body)}
  function debtsSubContent(kind){let debts=typeof activeDebts==='function'?activeDebts():arr('debts').filter(d=>d.status!=='Закрыт'); if(kind==='owe')debts=debts.filter(d=>(d.direction||'owe')==='owe'); if(kind==='owed')debts=debts.filter(d=>d.direction==='owed_to_me'); if(kind==='closed')debts=arr('debts').filter(d=>d.status==='Закрыт'); return panelList(kind==='owe'?'Кому я должен':kind==='owed'?'Кто должен мне':kind==='closed'?'Закрытые долги':'Все долги',debts.map(d=>typeof rowDebt==='function'?rowDebt(d):rowSimple('Долг',d.person,rub(d.amount),'editDebt',d.id)),'addDebt')}
  function debtsPageFinal(){const d=getDrill('debts'); const all=typeof activeDebts==='function'?activeDebts():arr('debts'); const cards=[['owe','Я должен','↘',rub(all.filter(x=>(x.direction||'owe')==='owe').reduce((a,b)=>a+num2(b.amount),0))],['owed','Мне должны','↗',rub(all.filter(x=>x.direction==='owed_to_me').reduce((a,b)=>a+num2(b.amount),0))],['closed','Закрытые','✓',`${arr('debts').filter(x=>x.status==='Закрыт').length} записей`],['all','Все долги','▣',`${all.length} записей`]].map(f=>card('debts',f[0],f[1],f[2],f[3],'')); if(!d.folder)return rootPage('debts','Долги','Долги разнесены по подпапкам.',cards.join('')); return layout('Долги','Отдельная подпапка долгов.',subHeader('debts','Долги','Назад к подпапкам')+debtsSubContent(d.folder))}
  function tripsPageFinal(){const d=getDrill('trips'); const cards=[['all','Все поездки','✈',`${arr('trips').length} записей`],['future','Будущие','↗','планы'],['budget','Бюджет','₽','расходы']].map(f=>card('trips',f[0],f[1],f[2],f[3],'')); if(!d.folder)return rootPage('trips','Путешествия','Поездки, планы и бюджеты в подпапках.',cards.join('')); return layout('Путешествия','Отдельная подпапка.',subHeader('trips','Путешествия','Назад к подпапкам')+(typeof tripsPanel==='function'?tripsPanel():panelList('Поездки',arr('trips').map(t=>rowSimple('Поездка',t.title||'',t.note||'','editTrip',t.id)),'addTrip')))}
  function importPageFinal(){const d=getDrill('import'); const cards=[['csv','CSV импорт','⇣','банковские операции'],['rules','Правила категорий','▦','автокатегории'],['history','История','◷','журнал импортов']].map(f=>card('import',f[0],f[1],f[2],f[3],'')); if(!d.folder)return rootPage('import','Импорт','Импорт и проверка данных в подпапках.',cards.join('')); return layout('Импорт','Отдельная подпапка.',subHeader('import','Импорт','Назад к подпапкам')+(typeof importPage==='function'&&importPage!==importPageFinal?importPage():empty2('Импорт недоступен')))}
  function diagnosticsPageFinal(){const d=getDrill('diagnostics'); const cards=[['system','Система','⚙','проверка'],['archive','Архив','▣','скрыто'],['storage','Данные','▤','localStorage']].map(f=>card('diagnostics',f[0],f[1],f[2],f[3],'')); if(!d.folder)return rootPage('diagnostics','Диагностика','Служебные разделы спрятаны по подпапкам.',cards.join('')); let body=d.folder==='archive'&&typeof archivePage==='function'?archivePage():(typeof diagnosticsPage==='function'&&diagnosticsPage!==diagnosticsPageFinal?diagnosticsPage():empty2('Раздел диагностики')); return layout('Диагностика','Отдельная подпапка.',subHeader('diagnostics','Диагностика','Назад к подпапкам')+body)}

  function derivedDebtFromOp(o){const text=`${o.category||''} ${o.note||''}`; if(!/долг|за[её]м|займ|вернуть|вернул|вернули|одолж|кредит|ипотек/i.test(text))return null; const incoming=String(o.type||'expense')==='income'; return {id:'op_'+(o.id||uid2()),source:'operation',operationId:o.id,direction:incoming?'owed_to_me':'owe',person:o.note||o.category||'Долг из операции',amount:num2(o.amount),due:o.date||today2(),status:'Активен',note:'Определено по операции'}}
  function findDebtFinal(id){id=String(id||''); let d=arr('debts').find(x=>String(x.id)===id); if(d)return {debt:d,explicit:true}; for(const o of arr('operations')){const dd=derivedDebtFromOp(o); if(dd&&String(dd.id)===id)return {debt:dd,explicit:false,operation:o}} return {debt:null,explicit:false}}
  function hideDebtFinal(id){state.settings.hiddenDebtIds=Array.isArray(state.settings.hiddenDebtIds)?state.settings.hiddenDebtIds:[]; id=String(id||''); if(id&&!state.settings.hiddenDebtIds.includes(id))state.settings.hiddenDebtIds.push(id)}
  function editDebtDateFinal(id){const found=findDebtFinal(id), d=found.debt; if(!d){toast2('Долг не найден');return true} const label=(d.direction==='owed_to_me')?'Когда должны вернуть':'Когда отдать'; const title=(d.direction==='owed_to_me')?'Редактировать долг — мне должны':'Редактировать долг — я должен'; openModal(title,`<div class="form-grid"><div class="field"><label>Тип</label><select id="f_direction"><option value="owe" ${(d.direction||'owe')==='owe'?'selected':''}>Я должен</option><option value="owed_to_me" ${d.direction==='owed_to_me'?'selected':''}>Мне должны</option></select></div>${field('Кто / кому','f_person',d.person||'')}${field('Сумма','f_amount',d.amount||0,'number')}${field(label,'f_due',d.due||today2(),'date')}<div class="field"><label>Статус</label><select id="f_status"><option ${d.status==='Активен'?'selected':''}>Активен</option><option ${d.status==='Ожидаю'?'selected':''}>Ожидаю</option><option ${d.status==='Закрыт'?'selected':''}>Закрыт</option></select></div>${area('Комментарий','f_note',d.note||'')}</div><div class="actions"><button type="button" class="btn" data-action="saveDebtDateFinal" data-id="${q(id)}">Сохранить дату</button><button type="button" class="mini-btn red" data-action="deleteDebt" data-id="${q(id)}">Удалить</button></div>`); return true}
  function saveDebtDateFinal(id){const found=findDebtFinal(id), d=found.debt; if(!d){toast2('Долг не найден');return true} const data={direction:formVal('f_direction')||'owe',person:formVal('f_person')||'Долг',amount:num2(formVal('f_amount')),due:formVal('f_due')||today2(),status:formVal('f_status')||'Активен',note:formVal('f_note')||''}; if(found.explicit){Object.assign(d,data)}else{hideDebtFinal(id); arr('debts').unshift({id:uid2(),...data})} try{closeModal()}catch(e){} save2(true); toast2('Дата долга сохранена'); render(); return true}

  function openDailySubconsciousInterviewFinal(){
    const qd=typeof questionOfDay==='function'?questionOfDay():'Что я уже знаю, но пока не хочу признать?'; const quote=typeof dailyQuote==='function'?dailyQuote().quote:'Честность с собой — начало порядка.';
    openModal('Интервью с подсознанием',`<div class="notebook-paper"><div class="notebook-quote-line">“${q(quote)}”</div><div class="field"><label>Вопрос дня</label><textarea id="f_question">${q(qd)}</textarea></div><div class="field"><label>Что я сейчас чувствую?</label><textarea id="f_feel"></textarea></div><div class="field"><label>Чего я избегаю?</label><textarea id="f_avoid"></textarea></div><div class="field"><label>Что я уже знаю, но не признаю?</label><textarea id="f_truth"></textarea></div><div class="field"><label>Один маленький шаг</label><textarea id="f_step"></textarea></div></div><div class="actions"><button type="button" class="btn" data-action="saveDailySubconsciousInterviewFinal">Сохранить в заметки</button></div>`); return true
  }
  function saveDailySubconsciousInterviewFinal(){const rec={id:uid2(),date:today2(),quote:typeof dailyQuote==='function'?dailyQuote().quote:'',question:formVal('f_question'),feel:formVal('f_feel'),avoid:formVal('f_avoid'),truth:formVal('f_truth'),step:formVal('f_step')}; arr('subconsciousDiary').unshift(rec); arr('notes').unshift({id:uid2(),title:'Интервью с подсознанием',folder:'Дневник',text:`Цитата: ${rec.quote}\nВопрос: ${rec.question}\nЧувствую: ${rec.feel}\nИзбегаю: ${rec.avoid}\nЗнаю, но не признаю: ${rec.truth}\nШаг: ${rec.step}`,tags:['рефлексия','подсознание','интервью','цитата'],date:rec.date}); let h=arr('habits').find(x=>/интервью с подсознанием/i.test(x.title||'')); if(!h){h={id:uid2(),title:'Интервью с подсознанием',marks:{},note:'Ежедневный дневник'}; arr('habits').unshift(h)} h.marks=h.marks||{}; h.marks[today2()]=true; try{closeModal()}catch(e){} save2(true); toast2('Интервью сохранено в заметки'); render(); return true}

  window.setDrillFolder=setDrill; window.backDrill=backDrill; window.editDebtDateFinal=editDebtDateFinal; window.saveDebtDateFinal=saveDebtDateFinal; window.openDailySubconsciousInterviewFinal=openDailySubconsciousInterviewFinal; window.saveDailySubconsciousInterviewFinal=saveDailySubconsciousInterviewFinal;
  try{Object.assign(windowActions||{}, {setDrillFolder:setDrill,backDrill,editDebtDateFinal,saveDebtDateFinal,openDailySubconsciousInterviewFinal,saveDailySubconsciousInterviewFinal});}catch(e){}
  const previousForceFinal=window.SBOS_FORCE_ACTION;
  window.SBOS_FORCE_ACTION=function(action,id,el){try{action=action||(el&&el.dataset&&el.dataset.action)||''; id=id||(el&&el.dataset&&el.dataset.id)||''; if(action==='setDrillFolder')return setDrill(el.dataset.section,el.dataset.folder,el.dataset.sub),true; if(action==='backDrill')return backDrill(el.dataset.section),true; if(action==='editDebt')return editDebtDateFinal(id); if(action==='saveDebtDateFinal')return saveDebtDateFinal(id); if(action==='openDailySubconsciousInterviewFinal')return openDailySubconsciousInterviewFinal(); if(action==='saveDailySubconsciousInterviewFinal')return saveDailySubconsciousInterviewFinal(); return previousForceFinal?previousForceFinal(action,id,el):false}catch(e){console.error('[drilldown final action]',action,e);toast2('Ошибка кнопки: '+(e.message||action));return true}};
  document.addEventListener('click',function(ev){const el=ev.target&&ev.target.closest&&ev.target.closest('[data-action],[data-go]'); if(!el)return; if(el.dataset.go){ensureDrill(); const tgt=el.dataset.go; if(state.settings.folderDrill[tgt])state.settings.folderDrill[tgt]={folder:'',sub:''}; save2(true); return;} const a=el.dataset.action||''; if(['setDrillFolder','backDrill','editDebt','saveDebtDateFinal','openDailySubconsciousInterviewFinal','saveDailySubconsciousInterviewFinal'].includes(a)){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation(); window.SBOS_FORCE_ACTION(a,el.dataset.id||'',el);}},true);

  function injectDrillCss(){if(document.getElementById('folderDrillCss'))return; const st=document.createElement('style'); st.id='folderDrillCss'; st.textContent=`
    .drill-home{padding:20px}.drill-grid{grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}.drill-card{min-height:158px;text-align:left;align-items:flex-start;padding:18px}.drill-card p{margin:4px 0 0;color:#64748b;font-size:13px;line-height:1.35;z-index:1}.drill-card .pill{margin-top:auto}.drill-head{display:flex;align-items:center;justify-content:space-between}.drill-head h3{font-size:24px;margin:12px 0 4px}.drill-row{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:14px;align-items:center;background:#fbfdff;border:1px solid #e4edf8;border-radius:22px;padding:13px;overflow:hidden}.drill-row-main{min-width:0}.compact-row{margin:0 0 6px}.drill-img{width:170px;height:100px;object-fit:cover;border-radius:18px;border:1px solid #deebfb;background:#eef6ff}.notebook-card{background:linear-gradient(135deg,#fff,#f8fbff)}.notebook-quote,.notebook-question{border:1px solid #e3ecfb;border-radius:20px;background:#fff;padding:14px;margin-bottom:10px}.notebook-quote span,.notebook-question span{display:block;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;margin-bottom:6px}.notebook-quote b,.notebook-question b{font-size:16px}.notebook-paper{background:repeating-linear-gradient(#fff,#fff 31px,#eef4ff 32px);border:1px solid #e3ecfb;border-radius:22px;padding:14px}.notebook-quote-line{font-weight:900;color:#1d4ed8;margin-bottom:10px}@media(max-width:760px){.drill-row{grid-template-columns:1fr}.drill-img{width:100%;height:155px}.drill-head{align-items:flex-start;flex-direction:column}.drill-grid{grid-template-columns:1fr}}
  `; document.head.appendChild(st)}
  const oldRenderFinal=render;
  render=function(){ensureDrill(); try{renderNav()}catch(e){} const map={dashboard,finance:financePageFinal,budget:budgetPageFinal,goals:goalsPageFinal,planning:planningPageFinal,spheres:spheresPageFinal,habits:habitsPageFinal,debts:debtsPageFinal,trips:tripsPageFinal,import:importPageFinal,diagnostics:diagnosticsPageFinal}; try{$('#view').innerHTML=(map[page]||dashboard)()}catch(e){console.error('[folder drill render]',e); try{$('#view').innerHTML=layout('Second Brain OS','Ошибка рендера',`<section class="card"><h3>Ошибка</h3><p>${q(e.message)}</p></section>`)}catch(_){}} setTimeout(()=>{try{injectDrillCss(); if(typeof patchClickableButtons==='function')patchClickableButtons()}catch(e){}},0); return true};
  window.render=render;
  try{ensureDrill(); save2(true); injectDrillCss(); render(); console.log('[Second Brain OS]',BUILD)}catch(e){console.error('[folder drilldown final]',e)}
})();

/* ===== Folder Explorer UX Fix — only Planning + Life Spheres use folder/subfolder sheets ===== */
(function(){
  const FOLDER_BUILD='folder-explorer-only-planning-spheres-20260630';
  try{localStorage.setItem('secondBrainOS.currentBuild',FOLDER_BUILD)}catch(e){}
  const safe=v=>String(v??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[ch]));
  const rub=v=>{try{return typeof money==='function'?money(v):`${Math.round(Number(v)||0).toLocaleString('ru-RU')} ₽`}catch(e){return `${Math.round(Number(v)||0).toLocaleString('ru-RU')} ₽`}};
  const n=v=>Number(String(v??'').replace(/\s/g,'').replace(',','.'))||0;
  const month=()=>{try{return (state.settings&&state.settings.currentMonth)||monthKey()}catch(e){return new Date().toISOString().slice(0,7)}};
  const today=()=>{try{return todayKey()}catch(e){return new Date().toISOString().slice(0,10)}};
  const arr=k=>{ if(!state[k]||!Array.isArray(state[k])) state[k]=[]; return state[k]; };
  const saveQuiet=()=>{try{save(true)}catch(e){try{localStorage.setItem(STORE_KEY,JSON.stringify({state}))}catch(_){}}};
  const toastSafe=t=>{try{toast(t)}catch(e){console.log('[SBOS]',t)}};
  function monthTitle(m){try{return new Date(String(m||month())+'-01T12:00:00').toLocaleDateString('ru-RU',{month:'long',year:'numeric'})}catch(e){return m||month()}}
  function setup(){
    state.settings=state.settings||{};
    if(!state.settings.folderExplorer) state.settings.folderExplorer={planning:{folder:''},spheres:{folder:'',sub:''}};
    if(!state.settings.folderExplorer.planning) state.settings.folderExplorer.planning={folder:''};
    if(!state.settings.folderExplorer.spheres) state.settings.folderExplorer.spheres={folder:'',sub:''};
    ['tasks','calendarEvents','plannedPurchases','people','notes','habits','books','ideas','media','files','trips','subconsciousDiary'].forEach(arr);
  }
  function setExp(section,folder='',sub=''){ setup(); state.settings.folderExplorer[section]={folder:folder||'',sub:sub||''}; saveQuiet(); render(); }
  function backExp(section){ setup(); const d=state.settings.folderExplorer[section]||{}; if(d.sub)setExp(section,d.folder,''); else setExp(section,'',''); }
  function resetExp(section){ setup(); state.settings.folderExplorer[section]={folder:'',sub:''}; saveQuiet(); }
  function kind(v){v=String(v||'').trim(); if(/виш|wish|хотел/i.test(v))return 'Вишлист'; if(/плат|обяз/i.test(v))return 'Платёж'; if(/будущ/i.test(v))return 'Будущая трата'; if(/покуп/i.test(v))return 'Покупка'; return v||'Покупка'}
  function allPlan(){
    setup(); const out=[];
    arr('tasks').forEach(t=>{ if(String(t.status||'')==='Готово')return; out.push({src:'task',id:t.id,title:t.title||'Задача',folder:'tasks',type:'Задача',owner:t.owner||'Я',amount:0,month:String(t.due||month()).slice(0,7)||month(),date:t.due||'',time:t.time||'',meta:t.area||'Личное',image:t.image||'',link:''}); });
    arr('calendarEvents').forEach(e=>out.push({src:'event',id:e.id,title:e.title||'Событие',folder:'calendar',type:'Календарь',owner:e.owner||'Общее',amount:0,month:String(e.date||month()).slice(0,7)||month(),date:e.date||'',time:e.time||'',meta:e.area||'Календарь',image:e.image||'',link:''}));
    arr('plannedPurchases').forEach(p=>{ const k=kind(p.kind); const f=k==='Вишлист'?'wishlist':k==='Платёж'?'payments':k==='Будущая трата'?'future':'purchases'; out.push({src:'purchase',id:p.id,title:p.title||'Пункт',folder:f,type:k,owner:p.owner||'Я',amount:n(p.amount),month:p.month||month(),date:p.due||'',time:'',meta:`${p.category||'Без категории'} · ${p.includeInBudget!==false?'в прогнозе':'вне прогноза'}`,image:p.image||'',link:p.link||'',include:p.includeInBudget!==false&&p.active!==false}); });
    return out;
  }
  const planFolders=[
    ['tasks','Задачи','✓','дела, сроки, фокус'],['calendar','Календарь','◷','события и даты'],['wishlist','Вишлист','✦','желания: Я / Полина / Общее'],['payments','Платежи','₽','регулярные обязательства'],['future','Будущие траты','↗','расходы вперёд'],['purchases','Покупки','◇','покупки и картинки']
  ];
  function planList(id){return allPlan().filter(x=>x.folder===id)}
  function planSum(id){return planList(id).filter(x=>x.include!==false).reduce((a,b)=>a+n(b.amount),0)}
  function folderCard(section,folder,title,ico,count,desc,sub){
    return `<button type="button" class="os-folder" data-action="folderExplorerOpen" data-section="${safe(section)}" data-folder="${safe(folder)}" data-sub="${safe(sub||'')}"><span class="os-folder-tab"></span><div class="os-folder-ico">${safe(ico)}</div><div class="os-folder-title">${safe(title)}</div><div class="os-folder-count">${safe(count||'')}</div>${desc?`<div class="os-folder-desc">${safe(desc)}</div>`:''}</button>`;
  }
  function sheet(title,subtitle,body,back){
    return `<section class="folder-sheet"><div class="folder-sheet-head"><div>${back?`<button type="button" class="mini-btn" data-action="folderExplorerBack" data-section="${safe(back)}">← Назад</button>`:''}<h3>${safe(title)}</h3><p>${safe(subtitle||'')}</p></div></div>${body}</section>`;
  }
  function planRoot(){
    const total=allPlan().filter(x=>x.include!==false).reduce((a,b)=>a+n(b.amount),0);
    const cards=planFolders.map(f=>folderCard('planning',f[0],f[1],f[2],`${planList(f[0]).length} пунктов${planSum(f[0])?' · '+rub(planSum(f[0])):''}`,f[3])).join('');
    return layout('Планирование','Как папка на компьютере: внутри только подпапки, без общей простыни.',`<section class="folder-brief"><div><b>${allPlan().length}</b><span>всего пунктов</span></div><div><b>${rub(total)}</b><span>запланировано</span></div><div><b>${monthTitle(month())}</b><span>текущий месяц</span></div></section>${sheet('Папка “Планирование”','Выбери подпапку. На этом листе нет задач и покупок вперемешку — только структура.',`<div class="folder-desktop">${cards}</div>`,'')}`);
  }
  function rowPlanItem(x){
    if(x.src==='task' && typeof rowTask==='function'){return rowTask(arr('tasks').find(t=>String(t.id)===String(x.id))||x)}
    if(x.src==='event' && typeof rowEvent==='function'){return rowEvent(arr('calendarEvents').find(e=>String(e.id)===String(x.id))||x)}
    if(x.src==='purchase' && typeof rowPurchase==='function'){return rowPurchase(arr('plannedPurchases').find(p=>String(p.id)===String(x.id))||x)}
    const img=x.image?`<img class="folder-row-img" src="${safe(x.image)}" loading="lazy" onerror="this.remove()">`:'';
    const act=x.src==='task'?'editTask':x.src==='event'?'editEvent':'editPurchase';
    return `<div class="folder-item-row">${img}<div class="folder-item-main"><div><span class="pill blue">${safe(x.type)}</span><span class="pill ${x.owner==='Полина'?'violet':x.owner==='Общее'?'green':'blue'}">${safe(x.owner)}</span>${x.amount?`<span class="pill green">${rub(x.amount)}</span>`:''}</div><strong>${safe(x.title)}</strong><p>${safe(x.date||monthTitle(x.month))}${x.time?' · '+safe(x.time):''} · ${safe(x.meta||'')}</p></div><div class="button-row"><button class="mini-btn" data-action="${safe(act)}" data-id="${safe(x.id)}">Ред.</button></div></div>`;
  }
  function planFolderPage(id){
    const f=planFolders.find(x=>x[0]===id)||planFolders[0]; const rows=planList(id); const add=id==='tasks'?'addTask':id==='calendar'?'addEvent':id==='wishlist'?'addWishlistItem':id==='payments'?'addPaymentItem':'addPurchase';
    return layout('Планирование · '+f[1],f[3],sheet(f[1],`Отдельный лист папки “${f[1]}”. Здесь только эта категория.`, `<div class="folder-action-line"><div><b>${rows.length}</b><span>пунктов</span></div><div><b>${rub(planSum(id))}</b><span>сумма</span></div><button class="btn secondary" data-action="${safe(add)}">Добавить</button></div><div class="folder-list-only">${rows.map(rowPlanItem).join('')||empty('В этой папке пока пусто')}</div>`, 'planning'));
  }
  function planningExplorerPage(){ setup(); const d=state.settings.folderExplorer.planning||{}; return d.folder?planFolderPage(d.folder):planRoot(); }

  const sphereFolders=[
    ['people','Люди','◌','контакты, подарки, разговоры'],['notes','Заметки','✎','мысли, ссылки, рефлексия'],['planning','Планирование','☷','задачи и расходы'],['habits','Привычки','◷','ритм и ежедневное интервью'],['books','Книги','▣','цитаты и выводы'],['ideas','Идеи','✦','замыслы и проекты'],['personal','Личная жизнь','♡','фильмы, сериалы, личное'],['documents','Документы','▤','файлы и ссылки'],['trips','Путешествия','✈','поездки и бюджеты'],['diary','Дневник','◉','интервью с подсознанием'],['wishlist','Вишлист','✦','желания с владельцем'],['payments','Платежи','₽','обязательные расходы']
  ];
  const sphereSubs={
    people:[['all','Все люди','◌'],['birthdays','Дни рождения','◷'],['gifts','Подарки','✦'],['talks','Темы разговора','✎']],
    notes:[['all','Все заметки','✎'],['reflection','Рефлексия','◉'],['diary','Дневник','◑'],['ideas','Идеи','✦']],
    planning:planFolders.map(f=>[f[0],f[1],f[2]]),
    habits:[['interview','Интервью с подсознанием','◉'],['tracker','Трекер привычек','◷'],['month','Календарь месяца','▦']],
    books:[['all','Все книги','▣'],['quotes','Цитаты','“”'],['insights','Выводы','◎']],
    ideas:[['all','Все идеи','✦'],['active','В работе','↗'],['archive','Архив','▣']],
    personal:[['watch','Что посмотреть','▶'],['notes','Личное','♡'],['memory','Память','◌']],
    documents:[['all','Все документы','▤'],['links','Ссылки','↗'],['important','Важное','!']],
    trips:[['all','Все поездки','✈'],['future','Будущие','↗'],['budget','Бюджет','₽']],
    diary:[['today','Начать интервью','◉'],['all','Все записи','▣'],['reflection','Рефлексия','✎']],
    wishlist:[['all','Весь вишлист','✦'],['Я','Мой','Я'],['Полина','Полины','П'],['Общее','Общее','О']],
    payments:[['all','Все платежи','₽'],['Я','Мои','Я'],['Полина','Полины','П'],['Общее','Общие','О']]
  };
  function sphereCount(id){
    const map={people:arr('people').length,notes:arr('notes').length,planning:allPlan().length,habits:arr('habits').length,books:arr('books').length,ideas:arr('ideas').length,personal:arr('media').length,documents:arr('files').length,trips:arr('trips').length,diary:arr('subconsciousDiary').length,wishlist:planList('wishlist').length,payments:planList('payments').length};
    return map[id]||0;
  }
  function subCount(folder,sub){
    if(folder==='planning')return planList(sub).length;
    if(folder==='wishlist')return sub==='all'?planList('wishlist').length:planList('wishlist').filter(x=>x.owner===sub).length;
    if(folder==='payments')return sub==='all'?planList('payments').length:planList('payments').filter(x=>x.owner===sub).length;
    if(folder==='diary')return sub==='today'?1:arr('subconsciousDiary').length;
    if(folder==='people'){let l=arr('people'); if(sub==='birthdays')l=l.filter(p=>p.birthday); if(sub==='gifts')l=l.filter(p=>p.gifts); if(sub==='talks')l=l.filter(p=>p.talkIdeas); return l.length;}
    if(folder==='notes'){let l=arr('notes'); if(sub==='reflection')l=l.filter(x=>(x.tags||[]).includes('рефлексия')); if(sub==='diary')l=l.filter(x=>/дневник|подсознан|интервью/i.test([x.title,x.folder,(x.tags||[]).join(' ')].join(' '))); if(sub==='ideas')l=l.filter(x=>/иде/i.test([x.title,x.folder,(x.tags||[]).join(' ')].join(' '))); return l.length;}
    return sphereCount(folder);
  }
  function spheresRootPage(){
    const cards=sphereFolders.map(f=>folderCard('spheres',f[0],f[1],f[2],`${sphereCount(f[0])} записей`,f[3])).join('');
    return layout('Сферы жизни','Как на компьютере: открыл папку — внутри только папки по категориям.',sheet('Папка “Сферы жизни”','Сначала только основные папки. Никаких списков ниже, чтобы экран не уезжал вниз.',`<div class="folder-desktop">${cards}</div>`,''));
  }
  function sphereFolderPage(folder){
    const f=sphereFolders.find(x=>x[0]===folder)||sphereFolders[0]; const subs=sphereSubs[folder]||[['all','Все записи','☷']];
    const cards=subs.map(s=>folderCard('spheres',folder,s[1],s[2],`${subCount(folder,s[0])} записей`,'',s[0])).join('');
    return layout('Сферы жизни · '+f[1],f[3],sheet(`Папка “${f[1]}”`,'Внутри этой папки лежат подпапки. Содержимое появится только после выбора подпапки.',`<div class="folder-desktop">${cards}</div>`,'spheres'));
  }
  function listPanel(title,rows,add){return `<section class="card folder-content-card"><div class="card-head"><h3>${safe(title)}</h3>${add?`<button class="btn secondary" data-action="${safe(add)}">Добавить</button>`:''}</div><div class="note-list">${rows.join('')||empty('Пока пусто')}</div></section>`}
  function simple(label,title,sub,action,id){return `<div class="list-row"><span class="pill blue">${safe(label)}</span><div><div class="row-title">${safe(title)}</div><div class="row-sub">${safe(sub||'')}</div></div>${action?`<div class="button-row"><button class="mini-btn" data-action="${safe(action)}" data-id="${safe(id)}">Ред.</button></div>`:''}</div>`}
  function rowsNotes(filter){let l=arr('notes'); if(filter==='reflection')l=l.filter(x=>(x.tags||[]).includes('рефлексия')); if(filter==='diary')l=l.filter(x=>/дневник|подсознан|интервью/i.test([x.title,x.folder,(x.tags||[]).join(' ')].join(' '))); if(filter==='ideas')l=l.filter(x=>/иде/i.test([x.title,x.folder,(x.tags||[]).join(' ')].join(' '))); return l.map(x=>simple((x.tags||[])[0]||'Заметка',x.title||'Заметка',String(x.text||'').slice(0,160),'editNote',x.id));}
  function dailyPanel(){
    if(typeof dailyInterviewPanel==='function') return dailyInterviewPanel();
    return `<section class="card notebook-card"><h3>Интервью с подсознанием</h3><p class="muted">Ежедневная запись сохраняется в заметки.</p><button class="btn" data-action="openDailySubconsciousInterviewFinal">Начать интервью</button></section>`;
  }
  function sphereContentPage(folder,sub){
    const title=(sphereSubs[folder]||[]).find(x=>x[0]===sub)?.[1]||'Все'; let body='';
    if(folder==='planning') body=`<div class="folder-content-card">${planFolderPage(sub).replace(/^<div class="page">|<\/div>$/g,'')}</div>`;
    else if(folder==='wishlist'){let rows=planList('wishlist'); if(sub&&sub!=='all')rows=rows.filter(x=>x.owner===sub); body=listPanel(title,rows.map(rowPlanItem),'addWishlistItem');}
    else if(folder==='payments'){let rows=planList('payments'); if(sub&&sub!=='all')rows=rows.filter(x=>x.owner===sub); body=listPanel(title,rows.map(rowPlanItem),'addPaymentItem');}
    else if(folder==='diary') body=sub==='today'?dailyPanel():listPanel(title,arr('subconsciousDiary').map(e=>simple('Дневник',e.date||'',(e.truth||e.step||e.notebook||e.question||'').slice(0,160),'editDailySubconsciousInterview',e.id)),null);
    else if(folder==='habits') body=sub==='interview'?dailyPanel():(typeof habitsFolder==='function'?habitsFolder():listPanel(title,arr('habits').map(h=>simple('Привычка',h.title||h.name||'',h.note||'','editHabit',h.id)),'addHabit'));
    else if(folder==='people'){let l=arr('people'); if(sub==='birthdays')l=l.filter(p=>p.birthday); if(sub==='gifts')l=l.filter(p=>p.gifts); if(sub==='talks')l=l.filter(p=>p.talkIdeas); body=listPanel(title,l.map(p=>simple('Человек',p.name||'',p.birthday||p.gifts||p.talkIdeas||p.notes||'','editPerson',p.id)),'addPerson');}
    else if(folder==='notes') body=listPanel(title,rowsNotes(sub),'addNote');
    else if(folder==='books'){let l=arr('books'); if(sub==='quotes')body=listPanel(title,l.flatMap(b=>(b.quotes||[]).map(x=>simple('Цитата',b.title||'',x,'editBook',b.id))),'addBook'); else if(sub==='insights')body=listPanel(title,l.filter(b=>b.insight).map(b=>simple('Вывод',b.title||'',b.insight,'editBook',b.id)),'addBook'); else body=listPanel(title,l.map(b=>simple('Книга',b.title||'',b.insight||((b.quotes||[])[0])||'','editBook',b.id)),'addBook');}
    else if(folder==='ideas') body=listPanel(title,arr('ideas').map(i=>simple('Идея',i.title||'',i.note||i.text||'','editIdea',i.id)),'addIdea');
    else if(folder==='personal') body=(typeof personalFolder==='function'?personalFolder():listPanel(title,arr('media').map(m=>simple('Личное',m.title||'',m.note||'','editMedia',m.id)),'addMedia'));
    else if(folder==='documents') body=(typeof documentsFolder==='function'?documentsFolder():listPanel(title,arr('files').map(f=>simple('Документ',f.title||'',f.url||f.note||'','editFile',f.id)),'addFileNote'));
    else if(folder==='trips') body=(typeof tripsPanel==='function'?tripsPanel():listPanel(title,arr('trips').map(t=>simple('Поездка',t.title||'',t.budget?rub(t.budget):t.note||'','editTrip',t.id)),'addTrip'));
    return layout('Сферы жизни · '+title,'Отдельный лист выбранной подпапки.',sheet(title,'Только выбранная категория. Назад вернёт к подпапкам этой папки.',body,'spheres'));
  }
  function spheresExplorerPage(){ setup(); const d=state.settings.folderExplorer.spheres||{}; if(!d.folder)return spheresRootPage(); if(!d.sub)return sphereFolderPage(d.folder); return sphereContentPage(d.folder,d.sub); }
  function compactFolderStrip(section,items){return `<div class="dash-folder-strip">${items.map(i=>`<button type="button" data-go="${safe(section)}"><span>${safe(i[2])}</span><b>${safe(i[1])}</b><small>${safe(i[3]||'')}</small></button>`).join('')}</div>`}
  const baseDashboard=typeof dashboard==='function'?dashboard:null;
  function dashboardCompact(){
    if(!baseDashboard) return layout('Обзор','Главная сводка',`<section class="card">Главная</section>`);
    let html=baseDashboard();
    // Не ломаем дашборд: только добавляем компактный верхний блок быстрого доступа, чтобы папки не тянули экран вниз.
    const insert=`<section class="grid cols-2 dashboard-compact-folders" style="margin-top:18px"><article class="card"><div class="card-head"><h3>Планирование</h3><button class="link" data-go="planning">Открыть папку →</button></div>${compactFolderStrip('planning',planFolders.slice(0,6).map(f=>[f[0],f[1],f[2],`${planList(f[0]).length}`]))}</article><article class="card"><div class="card-head"><h3>Сферы жизни</h3><button class="link" data-go="spheres">Открыть папку →</button></div>${compactFolderStrip('spheres',sphereFolders.slice(0,8).map(f=>[f[0],f[1],f[2],`${sphereCount(f[0])}`]))}</article></section>`;
    return html.replace('</div>',insert+'</div>');
  }
  function injectCss(){ if(document.getElementById('folderExplorerOnlyCss'))return; const st=document.createElement('style'); st.id='folderExplorerOnlyCss'; st.textContent=`
    .folder-brief{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-bottom:16px}.folder-brief>div{border:1px solid #e2ecf8;background:rgba(255,255,255,.86);border-radius:22px;padding:14px;box-shadow:0 12px 34px rgba(15,23,42,.06)}.folder-brief b{display:block;font-size:22px;color:#0f172a}.folder-brief span{font-size:12px;color:#64748b}.folder-sheet{background:linear-gradient(145deg,#ffffff,#f8fbff);border:1px solid #dfeaf7;border-radius:30px;padding:18px;box-shadow:0 20px 60px rgba(15,23,42,.08)}.folder-sheet-head{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px}.folder-sheet-head h3{margin:8px 0 4px;font-size:24px}.folder-sheet-head p{margin:0;color:#64748b}.folder-desktop{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:14px}.os-folder{position:relative;min-height:142px;border:1px solid #d9e8f8;border-radius:26px;padding:18px;text-align:left;background:linear-gradient(145deg,#fff,#f4f9ff);box-shadow:0 16px 38px rgba(37,99,235,.08);cursor:pointer;transition:.18s transform,.18s box-shadow,.18s border-color;overflow:hidden}.os-folder:hover{transform:translateY(-2px);border-color:#93c5fd;box-shadow:0 22px 56px rgba(37,99,235,.14)}.os-folder-tab{position:absolute;top:0;left:20px;width:58px;height:16px;border-radius:0 0 12px 12px;background:linear-gradient(135deg,#bfdbfe,#dbeafe)}.os-folder-ico{width:44px;height:44px;border-radius:16px;display:grid;place-items:center;background:linear-gradient(135deg,#e0f2fe,#eff6ff);font-weight:900;color:#1d4ed8;margin:8px 0 14px}.os-folder-title{font-weight:900;font-size:17px;color:#0f172a}.os-folder-count{font-size:12px;color:#2563eb;margin-top:4px}.os-folder-desc{font-size:12px;color:#64748b;margin-top:7px;line-height:1.32}.folder-action-line{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-bottom:14px}.folder-action-line>div,.folder-content-card{border:1px solid #e2ecf8;background:#fff;border-radius:22px;padding:14px}.folder-action-line b{display:block;font-size:22px}.folder-action-line span{font-size:12px;color:#64748b}.folder-list-only{display:grid;gap:10px}.folder-item-row{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:14px;align-items:center;border:1px solid #e2ecf8;background:#fff;border-radius:22px;padding:12px}.folder-row-img{width:150px;height:94px;object-fit:cover;border-radius:18px}.folder-item-main p{margin:4px 0 0;color:#64748b;font-size:13px}.dash-folder-strip{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.dash-folder-strip button{border:1px solid #e2ecf8;background:#fbfdff;border-radius:18px;padding:10px;text-align:left;cursor:pointer}.dash-folder-strip span{display:grid;place-items:center;width:26px;height:26px;border-radius:10px;background:#eaf4ff;color:#1d4ed8;font-weight:900;margin-bottom:6px}.dash-folder-strip b{display:block;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.dash-folder-strip small{color:#64748b;font-size:11px}@media(max-width:760px){.folder-brief,.folder-action-line{grid-template-columns:1fr}.folder-desktop{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.os-folder{min-height:128px;padding:14px}.folder-item-row{grid-template-columns:1fr}.folder-row-img{width:100%;height:150px}.dash-folder-strip{grid-template-columns:repeat(2,minmax(0,1fr))}}
  `; document.head.appendChild(st); }
  const prevForce=window.SBOS_FORCE_ACTION;
  window.SBOS_FORCE_ACTION=function(action,id,el){
    try{
      action=action||(el&&el.dataset&&el.dataset.action)||'';
      if(action==='folderExplorerOpen'){setExp(el.dataset.section,el.dataset.folder,el.dataset.sub||'');return true;}
      if(action==='folderExplorerBack'){backExp(el.dataset.section);return true;}
      if(prevForce)return prevForce(action,id,el);
      return false;
    }catch(e){console.error('[folder explorer action]',e);toastSafe('Ошибка: '+(e.message||action));return true;}
  };
  document.addEventListener('click',function(ev){
    const el=ev.target&&ev.target.closest&&ev.target.closest('[data-action],[data-go]'); if(!el)return;
    if(el.dataset.go && (el.dataset.go==='planning'||el.dataset.go==='spheres')){resetExp(el.dataset.go);return;}
    const a=el.dataset.action||'';
    if(a==='folderExplorerOpen'||a==='folderExplorerBack'){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();window.SBOS_FORCE_ACTION(a,el.dataset.id||'',el);}
  },true);
  const previousRender=render;
  render=function(){
    setup(); try{renderNav()}catch(e){}
    const map={dashboard:dashboardCompact,finance:financePage,budget:budgetPage,goals:goalsPage,planning:planningExplorerPage,spheres:spheresExplorerPage,habits:habitsPage,debts:debtsPage,trips:tripsPage,import:importPage,diagnostics:diagnosticsPage};
    try{document.querySelector('#view').innerHTML=(map[page]||dashboardCompact)()}catch(e){console.error('[folder explorer render]',e);document.querySelector('#view').innerHTML=layout('Second Brain OS','Ошибка',`<section class="card"><h3>Ошибка</h3><p>${safe(e.message)}</p></section>`)}
    setTimeout(()=>{try{injectCss(); if(typeof patchClickableButtons==='function')patchClickableButtons()}catch(e){}},0);
  };
  window.render=render;
  try{setup(); injectCss(); render(); console.log('[Second Brain OS]',FOLDER_BUILD)}catch(e){console.error('[folder explorer init]',e)}
})();


/* ===== Folder System V2 — real computer-like folders for Planning + Life Spheres only ===== */
(function(){
  const FOLDER_V2_BUILD='folder-system-v2-real-folders-20260630';
  try{localStorage.setItem('secondBrainOS.currentBuild',FOLDER_V2_BUILD)}catch(e){}
  const q=v=>String(v??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[ch]));
  const num2=v=>Number(String(v??'').replace(/\s/g,'').replace(',','.'))||0;
  const fmtMoney=v=>{try{return money(v)}catch(e){return Math.round(num2(v)).toLocaleString('ru-RU')+' ₽'}};
  const arr2=k=>{ if(!window.state) return []; if(!Array.isArray(state[k])) state[k]=[]; return state[k]; };
  const today2=()=>{try{return todayKey()}catch(e){return new Date().toISOString().slice(0,10)}};
  const month2=()=>{try{return (state.settings&&state.settings.currentMonth)||monthKey()}catch(e){return new Date().toISOString().slice(0,7)}};
  function monthName2(m){try{return new Date(String(m||month2())+'-01T12:00:00').toLocaleDateString('ru-RU',{month:'long',year:'numeric'})}catch(e){return m||month2()}}
  function addMonths2(m,shift){const [y,mo]=String(m||month2()).split('-').map(Number); const d=new Date(y||new Date().getFullYear(),(mo||1)-1+shift,1); return d.toISOString().slice(0,7)}
  function dateDaysFromNow(days){const d=new Date(); d.setDate(d.getDate()+days); return d.toISOString().slice(0,10)}
  function setupV2(){
    state.settings=state.settings||{};
    state.settings.fileExplorerV2=state.settings.fileExplorerV2||{planning:{folder:'',sub:''},spheres:{folder:'',sub:''}};
    state.settings.fileExplorerV2.planning=state.settings.fileExplorerV2.planning||{folder:'',sub:''};
    state.settings.fileExplorerV2.spheres=state.settings.fileExplorerV2.spheres||{folder:'',sub:''};
    ['tasks','calendarEvents','plannedPurchases','people','notes','habits','books','ideas','media','files','trips','subconsciousDiary'].forEach(arr2);
  }
  function saveV2(){try{save(true)}catch(e){try{localStorage.setItem(STORE_KEY,JSON.stringify(state))}catch(_){}}}
  function setV2(section,folder='',sub=''){setupV2(); state.settings.fileExplorerV2[section]={folder:folder||'',sub:sub||''}; saveV2(); render();}
  function resetV2(section){setupV2(); state.settings.fileExplorerV2[section]={folder:'',sub:''}; saveV2();}
  function backV2(section){setupV2(); const d=state.settings.fileExplorerV2[section]||{}; if(d.sub) setV2(section,d.folder,''); else setV2(section,'','');}
  function kind2(v){v=String(v||'').trim(); if(/виш|wish|хотел/i.test(v))return 'Вишлист'; if(/плат|обяз/i.test(v))return 'Платёж'; if(/будущ/i.test(v))return 'Будущая трата'; if(/покуп/i.test(v))return 'Покупка'; return v||'Покупка'}
  function allPlan2(){
    setupV2(); const out=[];
    arr2('tasks').forEach(t=>{ if(String(t.status||'')==='Готово')return; out.push({src:'task',id:t.id,title:t.title||'Задача',root:'tasks',type:'Задача',owner:t.owner||'Я',amount:0,month:String(t.due||month2()).slice(0,7)||month2(),date:t.due||'',time:t.time||'',area:t.area||'Личное',image:t.image||'',link:'',include:true,raw:t}); });
    arr2('calendarEvents').forEach(e=>out.push({src:'event',id:e.id,title:e.title||'Событие',root:'calendar',type:'Календарь',owner:e.owner||'Общее',amount:0,month:String(e.date||month2()).slice(0,7)||month2(),date:e.date||'',time:e.time||'',area:e.area||'Календарь',image:e.image||'',link:'',include:true,raw:e}));
    arr2('plannedPurchases').forEach(p=>{ const k=kind2(p.kind); const root=k==='Вишлист'?'wishlist':k==='Платёж'?'payments':k==='Будущая трата'?'future':'purchases'; out.push({src:'purchase',id:p.id,title:p.title||'Пункт',root,type:k,owner:p.owner||'Я',amount:num2(p.amount),month:p.month||month2(),date:p.due||'',time:'',area:p.category||'Без категории',image:p.image||'',link:p.link||'',include:p.includeInBudget!==false&&p.active!==false,raw:p}); });
    return out;
  }
  function inCurrentMonth(x){return String(x.month||'').slice(0,7)===month2() || String(x.date||'').slice(0,7)===month2()}
  function subFilterPlan(root,sub){
    let rows=allPlan2().filter(x=>x.root===root); const t=today2(); const m=month2();
    if(sub==='all') return rows;
    if(sub==='today') return rows.filter(x=>x.date===t);
    if(sub==='week') return rows.filter(x=>x.date && x.date>=t && x.date<=dateDaysFromNow(7));
    if(sub==='month'||sub==='this') return rows.filter(x=>String(x.month||x.date||'').slice(0,7)===m || String(x.date||'').slice(0,7)===m);
    if(sub==='next') return rows.filter(x=>String(x.month||x.date||'').slice(0,7)===addMonths2(m,1));
    if(sub==='later'||sub==='future') return rows.filter(x=>String(x.month||x.date||'').slice(0,7)>m || (x.date && x.date>dateDaysFromNow(7)));
    if(sub==='work') return rows.filter(x=>/работ/i.test(x.area));
    if(sub==='finance') return rows.filter(x=>/финанс|деньг|бюдж/i.test(x.area));
    if(sub==='personal') return rows.filter(x=>/личн|сем|отнош/i.test(x.area));
    if(sub==='mine'||sub==='Я') return rows.filter(x=>x.owner==='Я'||!x.owner);
    if(sub==='polina'||sub==='Полина') return rows.filter(x=>x.owner==='Полина');
    if(sub==='common'||sub==='Общее') return rows.filter(x=>x.owner==='Общее');
    if(sub==='forecast') return rows.filter(x=>x.include!==false);
    if(sub==='outside') return rows.filter(x=>x.include===false);
    if(sub==='image') return rows.filter(x=>!!x.image);
    if(sub==='links') return rows.filter(x=>!!x.link);
    if(sub==='active') return rows.filter(x=>x.raw&&x.raw.active!==false);
    return rows;
  }
  const planningRoots=[
    ['tasks','Задачи','✓','дела по срокам'],['calendar','Календарь','◷','события и даты'],['wishlist','Вишлист','✦','желания: Я / Полина / Общее'],['payments','Платежи','₽','обязательные расходы'],['future','Будущие траты','↗','расходы вперёд'],['purchases','Покупки','◇','покупки с картинками']
  ];
  const planningSubs={
    tasks:[['today','Сегодня','●'],['week','Неделя','7'],['month','Месяц','30'],['later','Позже','↗'],['work','Работа','Р'],['finance','Финансы','₽'],['personal','Личное','♡'],['all','Все задачи','☷']],
    calendar:[['today','Сегодня','●'],['week','Неделя','7'],['month','Месяц','30'],['future','Будущие','↗'],['all','Все события','☷']],
    wishlist:[['all','Весь вишлист','✦'],['mine','Мой','Я'],['polina','Полины','П'],['common','Общий','О'],['this','Этот месяц','30'],['later','Позже','↗']],
    payments:[['all','Все платежи','₽'],['mine','Мои','Я'],['polina','Полины','П'],['common','Общие','О'],['this','Этот месяц','30'],['active','Активные','●']],
    future:[['this','Этот месяц','30'],['next','Следующий месяц','↗'],['later','Позже','…'],['forecast','В прогнозе','✓'],['outside','Вне прогноза','×'],['all','Все траты','☷']],
    purchases:[['all','Все покупки','◇'],['mine','Мои','Я'],['polina','Полины','П'],['common','Общие','О'],['image','С картинкой','▧'],['links','Со ссылкой','↗']]
  };
  function sumRows(rows){return rows.filter(x=>x.include!==false).reduce((a,b)=>a+num2(b.amount),0)}
  function rootCount(id){return allPlan2().filter(x=>x.root===id).length}
  function rootSum(id){return sumRows(allPlan2().filter(x=>x.root===id))}
  function folderTile(section,folder,sub,title,ico,count,desc){
    return `<button type="button" class="fs-folder" data-action="folderV2Open" data-section="${q(section)}" data-folder="${q(folder)}" data-sub="${q(sub||'')}">
      <span class="fs-tab"></span><span class="fs-icon">${q(ico||'▣')}</span><b>${q(title)}</b><small>${q(count||'')}</small>${desc?`<em>${q(desc)}</em>`:''}
    </button>`;
  }
  function explorerHeader(section,title,subtitle,folderTitle){
    return `<section class="fs-header"><div>${folderTitle?`<button type="button" class="mini-btn" data-action="folderV2Back" data-section="${q(section)}">← Назад</button>`:''}<h2>${q(title)}</h2><p>${q(subtitle||'')}</p></div>${folderTitle?`<div class="fs-breadcrumb">${q(section==='planning'?'Планирование':'Сферы жизни')} / <b>${q(folderTitle)}</b></div>`:''}</section>`;
  }
  function planningRootV2(){
    const cards=planningRoots.map(f=>folderTile('planning',f[0],'',f[1],f[2],`${rootCount(f[0])} пунктов${rootSum(f[0])?' · '+fmtMoney(rootSum(f[0])):''}`,f[3])).join('');
    return layout('Планирование','',explorerHeader('planning','Планирование','Здесь только папки. Открой папку — внутри будут подпапки по категориям.')+`<section class="fs-grid">${cards}</section>`);
  }
  function planningSubfoldersV2(root){
    const r=planningRoots.find(x=>x[0]===root)||planningRoots[0];
    const cards=(planningSubs[root]||[['all','Все','☷']]).map(s=>{const rows=subFilterPlan(root,s[0]);return folderTile('planning',root,s[0],s[1],s[2],`${rows.length} пунктов${sumRows(rows)?' · '+fmtMoney(sumRows(rows)):''}`,'отдельный лист');}).join('');
    return layout('Планирование · '+r[1],'',explorerHeader('planning',r[1],'Это папка. Внутри только подпапки — содержимое появится после выбора подпапки.',r[1])+`<section class="fs-grid">${cards}</section>`);
  }
  function renderPlanRow(x){
    if(x.src==='task' && typeof rowTask==='function') return rowTask(x.raw||x);
    if(x.src==='event' && typeof rowEvent==='function') return rowEvent(x.raw||x);
    if(x.src==='purchase' && typeof rowPurchase==='function') return rowPurchase(x.raw||x);
    const img=x.image?`<img class="fs-row-img" src="${q(x.image)}" loading="lazy" onerror="this.remove()">`:'';
    const act=x.src==='task'?'editTask':x.src==='event'?'editEvent':'editPurchase';
    return `<div class="fs-row">${img}<div><div><span class="pill blue">${q(x.type)}</span><span class="pill ${x.owner==='Полина'?'violet':x.owner==='Общее'?'green':'blue'}">${q(x.owner)}</span>${x.amount?`<span class="pill green">${fmtMoney(x.amount)}</span>`:''}</div><strong>${q(x.title)}</strong><p>${q(x.date||monthName2(x.month))}${x.time?' · '+q(x.time):''} · ${q(x.area||'')}</p></div><button class="mini-btn" data-action="${q(act)}" data-id="${q(x.id)}">Ред.</button></div>`;
  }
  function planningContentV2(root,sub){
    const r=planningRoots.find(x=>x[0]===root)||planningRoots[0]; const s=(planningSubs[root]||[]).find(x=>x[0]===sub)||['all','Все','☷']; const rows=subFilterPlan(root,sub);
    const add=root==='tasks'?'addTask':root==='calendar'?'addEvent':root==='wishlist'?'addWishlistItem':root==='payments'?'addPaymentItem':'addPurchase';
    return layout('Планирование · '+s[1],'',explorerHeader('planning',s[1],`Отдельный лист: ${r[1]} / ${s[1]}. Здесь только выбранная категория.`,r[1]+' / '+s[1])+`<section class="card fs-content"><div class="card-head"><div><h3>${q(s[1])}</h3><p class="small muted">${rows.length} пунктов · ${fmtMoney(sumRows(rows))}</p></div><button class="btn secondary" data-action="${q(add)}">Добавить</button></div><div class="fs-list">${rows.map(renderPlanRow).join('')||empty('В этой подпапке пока пусто')}</div></section>`);
  }
  function planningPageV2(){setupV2(); const d=state.settings.fileExplorerV2.planning||{}; if(!d.folder)return planningRootV2(); if(!d.sub)return planningSubfoldersV2(d.folder); return planningContentV2(d.folder,d.sub);}

  const sphereRoots=[
    ['people','Люди','◌','контакты и отношения'],['notes','Заметки','✎','мысли и ссылки'],['planning','Планирование','☷','дела и расходы'],['habits','Привычки','◷','ритм и интервью'],['books','Книги','▣','цитаты и выводы'],['ideas','Идеи','✦','замыслы'],['personal','Личная жизнь','♡','личное и медиа'],['documents','Документы','▤','файлы'],['trips','Путешествия','✈','поездки'],['diary','Дневник','◉','подсознание'],['wishlist','Вишлист','✦','желания'],['payments','Платежи','₽','обязательные расходы']
  ];
  const sphereSubs={
    people:[['all','Все люди','◌'],['birthdays','Дни рождения','◷'],['gifts','Подарки','✦'],['talks','Темы разговора','✎']],
    notes:[['all','Все заметки','✎'],['reflection','Рефлексия','◉'],['diary','Дневник','◑'],['ideas','Идеи','✦'],['links','Ссылки','↗']],
    planning:planningRoots.map(x=>[x[0],x[1],x[2]]),
    habits:[['interview','Интервью с подсознанием','◉'],['tracker','Трекер','◷'],['month','Месяц','▦'],['all','Все привычки','☷']],
    books:[['all','Все книги','▣'],['quotes','Цитаты','“”'],['insights','Выводы','◎']],
    ideas:[['all','Все идеи','✦'],['active','В работе','↗'],['archive','Архив','▣']],
    personal:[['watch','Что посмотреть','▶'],['notes','Личное','♡'],['memory','Память','◌']],
    documents:[['all','Все документы','▤'],['links','Ссылки','↗'],['important','Важное','!']],
    trips:[['all','Все поездки','✈'],['future','Будущие','↗'],['budget','Бюджет','₽']],
    diary:[['today','Начать интервью','◉'],['all','Все записи','▣'],['reflection','Рефлексия','✎'],['quotes','Цитаты','“”']],
    wishlist:[['all','Весь вишлист','✦'],['mine','Мой','Я'],['polina','Полины','П'],['common','Общий','О']],
    payments:[['all','Все платежи','₽'],['mine','Мои','Я'],['polina','Полины','П'],['common','Общие','О']]
  };
  function countSphereRoot(id){
    if(id==='people')return arr2('people').length; if(id==='notes')return arr2('notes').length; if(id==='planning')return allPlan2().length; if(id==='habits')return arr2('habits').length; if(id==='books')return arr2('books').length; if(id==='ideas')return arr2('ideas').length; if(id==='personal')return arr2('media').length; if(id==='documents')return arr2('files').length; if(id==='trips')return arr2('trips').length; if(id==='diary')return arr2('subconsciousDiary').length; if(id==='wishlist')return rootCount('wishlist'); if(id==='payments')return rootCount('payments'); return 0;
  }
  function countSphereSub(root,sub){return listSphereRows(root,sub).length;}
  function simpleRow(label,title,meta,action,id){return `<div class="fs-row"><div><div><span class="pill blue">${q(label)}</span></div><strong>${q(title||'Без названия')}</strong><p>${q(meta||'')}</p></div>${action?`<button class="mini-btn" data-action="${q(action)}" data-id="${q(id)}">Ред.</button>`:''}</div>`}
  function listSphereRows(root,sub){
    let rows=[];
    if(root==='people'){let l=arr2('people'); if(sub==='birthdays')l=l.filter(p=>p.birthday); if(sub==='gifts')l=l.filter(p=>p.gifts); if(sub==='talks')l=l.filter(p=>p.talkIdeas); rows=l.map(p=>simpleRow('Человек',p.name,p.birthday||p.gifts||p.talkIdeas||p.notes,'editPerson',p.id));}
    else if(root==='notes'){let l=arr2('notes'); if(sub==='reflection')l=l.filter(n=>/рефлекс|подсозн|интерв/i.test([n.folder,n.title,(n.tags||[]).join(' ')].join(' '))); if(sub==='diary')l=l.filter(n=>/дневник|интерв/i.test([n.folder,n.title,(n.tags||[]).join(' ')].join(' '))); if(sub==='ideas')l=l.filter(n=>/иде/i.test([n.folder,n.title,(n.tags||[]).join(' ')].join(' '))); if(sub==='links')l=l.filter(n=>/https?:\/\//.test(n.text||n.note||'')); rows=l.map(n=>simpleRow('Заметка',n.title||n.folder,(n.text||n.note||'').slice(0,160),'editNote',n.id));}
    else if(root==='planning'){rows=allPlan2().filter(x=>x.root===sub).map(renderPlanRow);}
    else if(root==='habits'){ if(sub==='interview') rows=[dailyPanelV2()]; else rows=arr2('habits').map(h=>simpleRow('Привычка',h.title||h.name,h.note||'','editHabit',h.id)); }
    else if(root==='books'){let l=arr2('books'); if(sub==='quotes') rows=l.flatMap(b=>(b.quotes||[]).map(x=>simpleRow('Цитата',b.title,x,'editBook',b.id))); else if(sub==='insights') rows=l.filter(b=>b.insight).map(b=>simpleRow('Вывод',b.title,b.insight,'editBook',b.id)); else rows=l.map(b=>simpleRow('Книга',b.title,b.insight||((b.quotes||[])[0]),'editBook',b.id));}
    else if(root==='ideas'){rows=arr2('ideas').map(i=>simpleRow('Идея',i.title,i.note||i.text,'editIdea',i.id));}
    else if(root==='personal'){rows=arr2('media').map(m=>simpleRow('Личное',m.title,m.note,'editMedia',m.id));}
    else if(root==='documents'){rows=arr2('files').map(f=>simpleRow('Документ',f.title,f.url||f.note,'editFile',f.id));}
    else if(root==='trips'){rows=arr2('trips').map(t=>simpleRow('Поездка',t.title,t.budget?fmtMoney(t.budget):t.note,'editTrip',t.id));}
    else if(root==='diary'){ if(sub==='today') rows=[dailyPanelV2()]; else rows=arr2('subconsciousDiary').map(e=>simpleRow('Дневник',e.date||'',e.truth||e.step||e.notebook||e.question||'','editDailySubconsciousInterview',e.id));}
    else if(root==='wishlist'){let l=allPlan2().filter(x=>x.root==='wishlist'); if(sub==='mine')l=l.filter(x=>x.owner==='Я'||!x.owner); if(sub==='polina')l=l.filter(x=>x.owner==='Полина'); if(sub==='common')l=l.filter(x=>x.owner==='Общее'); rows=l.map(renderPlanRow);}
    else if(root==='payments'){let l=allPlan2().filter(x=>x.root==='payments'); if(sub==='mine')l=l.filter(x=>x.owner==='Я'||!x.owner); if(sub==='polina')l=l.filter(x=>x.owner==='Полина'); if(sub==='common')l=l.filter(x=>x.owner==='Общее'); rows=l.map(renderPlanRow);}
    return rows;
  }
  function dailyPanelV2(){
    return `<div class="fs-diary-card"><h3>Интервью с подсознанием</h3><p>Вопрос дня внутри интервью. Запись сохранится в дневник и заметки.</p><blockquote>Что я уже знаю, но пока не хочу признать?</blockquote><button class="btn" data-action="openDailySubconsciousInterview">Начать интервью</button></div>`;
  }
  function spheresRootV2(){
    const cards=sphereRoots.map(f=>folderTile('spheres',f[0],'',f[1],f[2],`${countSphereRoot(f[0])} записей`,f[3])).join('');
    return layout('Сферы жизни','',explorerHeader('spheres','Сферы жизни','Только папки. Открой сферу — внутри будут подпапки по категориям.')+`<section class="fs-grid">${cards}</section>`);
  }
  function spheresSubfoldersV2(root){
    const r=sphereRoots.find(x=>x[0]===root)||sphereRoots[0];
    const cards=(sphereSubs[root]||[['all','Все','☷']]).map(s=>folderTile('spheres',root,s[0],s[1],s[2],`${countSphereSub(root,s[0])} записей`,'отдельный лист')).join('');
    return layout('Сферы жизни · '+r[1],'',explorerHeader('spheres',r[1],'Это папка. Внутри только подпапки — никаких записей на этом уровне.',r[1])+`<section class="fs-grid">${cards}</section>`);
  }
  function spheresContentV2(root,sub){
    const r=sphereRoots.find(x=>x[0]===root)||sphereRoots[0]; const s=(sphereSubs[root]||[]).find(x=>x[0]===sub)||['all','Все','☷']; const rows=listSphereRows(root,sub); const addMap={people:'addPerson',notes:'addNote',books:'addBook',ideas:'addIdea',documents:'addFileNote',trips:'addTrip',wishlist:'addWishlistItem',payments:'addPaymentItem'}; const add=addMap[root]||'';
    return layout('Сферы жизни · '+s[1],'',explorerHeader('spheres',s[1],`Отдельный лист: ${r[1]} / ${s[1]}. Только выбранная категория.`,r[1]+' / '+s[1])+`<section class="card fs-content"><div class="card-head"><div><h3>${q(s[1])}</h3><p class="small muted">${rows.length} записей</p></div>${add?`<button class="btn secondary" data-action="${q(add)}">Добавить</button>`:''}</div><div class="fs-list">${rows.join('')||empty('В этой подпапке пока пусто')}</div></section>`);
  }
  function spheresPageV2(){setupV2(); const d=state.settings.fileExplorerV2.spheres||{}; if(!d.folder)return spheresRootV2(); if(!d.sub)return spheresSubfoldersV2(d.folder); return spheresContentV2(d.folder,d.sub);}

  function dashboardV2(){
    let s={actual:0,dailyLimit:0,toNext:0,daysToSalary:0,progress:0}; try{s=summary()}catch(e){}
    const plans=allPlan2().filter(x=>x.include!==false); const monthPlans=plans.filter(inCurrentMonth); const todayTasks=arr2('tasks').filter(t=>String(t.status||'')!=='Готово'&&t.due===today2()).slice(0,4);
    return layout('Обзор','Главный экран для контроля. Папки вынесены в Планирование и Сферы жизни.',`
      <section class="grid cols-4"><article class="card premium-hero-card"><h3>Фактический остаток</h3><div class="value">${fmtMoney(s.actual||0)}</div><p class="small muted">до зарплаты ${q(s.daysToSalary||0)} дн.</p></article><article class="card"><h3>Можно тратить в день</h3><div class="value sm">${fmtMoney(s.dailyLimit||0)}</div><p class="small muted">с учётом плана</p></article><article class="card"><h3>План расходов</h3><div class="value sm">${fmtMoney(sumRows(monthPlans))}</div><p class="small muted">на ${q(monthName2(month2()))}</p></article><article class="card"><h3>Активные долги</h3><div class="value sm">${(typeof activeDebts==='function'?activeDebts():arr2('debts')).filter(d=>String(d.status||'Активен')!=='Закрыт').length}</div><p class="small muted">открытых записей</p></article></section>
      <section class="grid cols-2" style="margin-top:18px"><article class="card"><div class="card-head"><h3>Сегодня</h3><button class="link" data-go="planning">Планирование →</button></div><div class="fs-list compact">${todayTasks.map(t=>typeof rowTask==='function'?rowTask(t):simpleRow('Задача',t.title,t.due,'editTask',t.id)).join('')||empty('Сегодня задач нет')}</div></article><article class="card"><div class="card-head"><h3>Быстрые папки</h3><span class="pill blue">без перегруза</span></div><div class="fs-quick"><button data-go="planning">☷<b>Планирование</b><small>${allPlan2().length} пунктов</small></button><button data-go="spheres">✣<b>Сферы жизни</b><small>${sphereRoots.length} папок</small></button><button data-go="finance">◔<b>Финансы</b><small>баланс и операции</small></button><button data-go="budget">◉<b>Бюджет</b><small>${fmtMoney(sumRows(monthPlans))}</small></button></div></article></section>
      <section class="grid cols-3" style="margin-top:18px"><article class="card"><div class="card-head"><h3>Вишлист</h3><button class="link" data-action="folderV2Open" data-section="planning" data-folder="wishlist">Открыть →</button></div>${allPlan2().filter(x=>x.root==='wishlist').slice(0,3).map(renderPlanRow).join('')||empty('Вишлист пуст')}</article><article class="card"><div class="card-head"><h3>Платежи</h3><button class="link" data-action="folderV2Open" data-section="planning" data-folder="payments">Открыть →</button></div>${allPlan2().filter(x=>x.root==='payments').slice(0,3).map(renderPlanRow).join('')||empty('Платежей нет')}</article><article class="card"><div class="card-head"><h3>Привычки</h3><button class="link" data-go="habits">Все →</button></div>${arr2('habits').slice(0,4).map(h=>simpleRow('Привычка',h.title||h.name,h.note||'','editHabit',h.id)).join('')||empty('Привычек нет')}</article></section>
    `);
  }
  function injectV2Css(){ if(document.getElementById('folderSystemV2Css'))return; const st=document.createElement('style'); st.id='folderSystemV2Css'; st.textContent=`
    .fs-header{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;margin:0 0 18px}.fs-header h2{font-size:34px;line-height:1;margin:8px 0 6px}.fs-header p{margin:0;color:#64748b;font-size:15px}.fs-breadcrumb{padding:10px 14px;border:1px solid #dbeafe;border-radius:18px;background:rgba(255,255,255,.8);color:#64748b}.fs-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(205px,1fr));gap:16px}.fs-folder{position:relative;min-height:150px;border:1px solid #d9e8f8;border-radius:28px;padding:20px 18px;text-align:left;background:linear-gradient(145deg,#fff,#f4f9ff);box-shadow:0 18px 48px rgba(37,99,235,.08);cursor:pointer;transition:.18s ease;overflow:hidden}.fs-folder:before{content:"";position:absolute;right:-24px;bottom:-24px;width:94px;height:72px;border-radius:38px;background:#e4f1ff}.fs-folder:hover{transform:translateY(-2px);border-color:#93c5fd;box-shadow:0 24px 66px rgba(37,99,235,.15)}.fs-tab{position:absolute;top:0;left:22px;width:74px;height:18px;border-radius:0 0 14px 14px;background:linear-gradient(135deg,#bfdbfe,#dbeafe)}.fs-icon{display:grid;place-items:center;width:42px;height:42px;border-radius:16px;background:linear-gradient(135deg,#e0f2fe,#eff6ff);color:#1d4ed8;font-weight:900;margin:10px 0 16px}.fs-folder b{display:block;font-size:17px;color:#0f172a}.fs-folder small{display:block;margin-top:6px;color:#2563eb;font-weight:800}.fs-folder em{display:block;margin-top:8px;color:#64748b;font-size:12px;font-style:normal;line-height:1.25}.fs-content{border-radius:30px}.fs-list{display:grid;gap:10px}.fs-list.compact{gap:8px}.fs-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:center;border:1px solid #e2ecf8;background:#fff;border-radius:22px;padding:12px}.fs-row p{margin:5px 0 0;color:#64748b;font-size:13px}.fs-row-img{width:150px;height:96px;object-fit:cover;border-radius:18px;margin-right:12px}.fs-diary-card{border:1px solid #dbeafe;border-radius:26px;background:linear-gradient(145deg,#fff,#f7fbff);padding:20px}.fs-diary-card blockquote{margin:12px 0;padding:14px;border-left:4px solid #3b82f6;background:#eff6ff;border-radius:14px;color:#1e3a8a}.fs-quick{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.fs-quick button{border:1px solid #e2ecf8;border-radius:20px;background:#fff;padding:14px;text-align:left;cursor:pointer}.fs-quick button>b{display:block;margin-top:6px}.fs-quick button>small{display:block;color:#64748b;margin-top:4px}@media(max-width:760px){.fs-header{display:block}.fs-header h2{font-size:28px}.fs-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.fs-folder{min-height:132px;padding:16px}.fs-folder b{font-size:14px}.fs-row{grid-template-columns:1fr}.fs-quick{grid-template-columns:1fr}.fs-row-img{width:100%;height:150px}}
  `; document.head.appendChild(st); }
  const prevForceV2=window.SBOS_FORCE_ACTION;
  window.SBOS_FORCE_ACTION=function(action,id,el){
    try{ action=action||(el&&el.dataset&&el.dataset.action)||'';
      if(action==='folderV2Open'){setV2(el.dataset.section,el.dataset.folder,el.dataset.sub||''); return true;}
      if(action==='folderV2Back'){backV2(el.dataset.section); return true;}
      if(prevForceV2) return prevForceV2(action,id,el);
      return false;
    }catch(e){console.error('[folder v2 action]',e); try{toast('Ошибка: '+e.message)}catch(_){alert('Ошибка: '+e.message)} return true;}
  };
  document.addEventListener('click',function(ev){const el=ev.target&&ev.target.closest&&ev.target.closest('[data-action],[data-go]'); if(!el)return; if(el.dataset.go==='planning'||el.dataset.go==='spheres'){resetV2(el.dataset.go);} const a=el.dataset.action||''; if(a==='folderV2Open'||a==='folderV2Back'){ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation(); window.SBOS_FORCE_ACTION(a,el.dataset.id||'',el);}},true);
  const oldRenderV2=window.render||render;
  render=function(){setupV2(); try{renderNav()}catch(e){} const map={dashboard:dashboardV2,finance:financePage,budget:budgetPage,goals:goalsPage,planning:planningPageV2,spheres:spheresPageV2,habits:habitsPage,debts:debtsPage,trips:tripsPage,import:importPage,diagnostics:diagnosticsPage}; try{document.querySelector('#view').innerHTML=(map[page]||dashboardV2)()}catch(e){console.error('[folder system v2 render]',e); document.querySelector('#view').innerHTML=layout('Second Brain OS','Ошибка',`<section class="card"><h3>Ошибка</h3><p>${q(e.message)}</p></section>`)} setTimeout(()=>{try{injectV2Css(); if(typeof patchClickableButtons==='function')patchClickableButtons()}catch(e){}},0); return true;};
  window.render=render; window.planningPage=planningPageV2; window.spheresPage=spheresPageV2;

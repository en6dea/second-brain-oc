...
const SBOS_V46 = 'v46-3-finance-ui-polish-safe-20260630';
const SBOS_V46_DISPLAY = 'V46.3 FINANCE UI POLISH';
...
function v46PatchVisibleVersion(){
  try{
    document.title='Second Brain OS — V46.3 Finance UI Polish';
    ...
    document.querySelectorAll('.sidebar-status,.v403-status,.app-version,.version-badge,[data-version],[data-build-version]').forEach(box=>{ box.setAttribute('data-build-version',SBOS_V46_DISPLAY); const b=box.matches('b')?box:box.querySelector('b'); if(b) b.textContent='V46.3'; const s=box.querySelector('.sync-dot, small:nth-child(1), span'); if(s && /UPDATE SAFE|DATA GUARD|PRODUCT ENGINE|PERSONAL OS|FINANCE MENTOR|ANALYTICS PRO|VERSION LOCK|V4[0-6]/i.test(String(s.textContent||''))){ s.textContent='FINANCE UI POLISH'; } });
...
function forceVersionBadgeRepair(){ v46PatchVisibleVersion(); try{ toast('Версия обновлена: V46.3'); }catch(_e){} }
try{
  v46EnsureRoadmapState();
...
/* === Second Brain OS V46.3 — Finance UI Polish Safe Layer === */
const SBOS_V463 = 'v46-3-finance-ui-polish-safe-20260630';
const SBOS_V463_DISPLAY = 'V46.3 FINANCE UI POLISH';
...
function v463CoachCard(f){
  ...
  return `<section class="card panel v463-coach"><div class="section-head"><div><div class="page-label">Finance Coach</div><h3>Что сделать с деньгами сейчас</h3><p class="sub">Простые шаги без сложной финансовой теории.</p></div><span class="tag green">V46.3</span></div><div class="v463-coach-grid">${items.map(x=>`<article class="v463-coach-item ${x[0]}"><i></i><div><b>${esc(x[1])}</b><p>${esc(x[2])}</p></div><button class="ghost small" ${x[3]==='addIncome'?'data-action="addIncome"':`data-page="${x[3]}"`}>${esc(x[4])}</button></article>`).join('')}</div></section>`;
}
...
function v463PatchVisibleVersion(){
  try{
    ...
    document.querySelectorAll('.sidebar-status,.v403-status,.app-version,.version-badge,[data-version],[data-build-version]').forEach(box=>{ box.setAttribute('data-build-version',SBOS_V463_DISPLAY); const b=box.matches('b')?box:box.querySelector('b'); if(b) b.textContent='V46.3'; const s=box.querySelector('.sync-dot, small:nth-child(1), span'); if(s && /UPDATE SAFE|DATA GUARD|PRODUCT ENGINE|PERSONAL OS|FINANCE MENTOR|ANALYTICS PRO|VERSION LOCK|FINANCIAL ROADMAP|PREMIUM DASHBOARD|V4[0-8]/i.test(String(s.textContent||''))){ s.textContent='FINANCE UI POLISH'; } });
  ...
  }catch(e){ console.warn('V46.3 version patch failed', e); }
}
function forceVersionBadgeRepair(){ v463PatchVisibleVersion(); try{ toast('Версия обновлена: V46.3'); }catch(_e){} }
try{
  injectV463Styles();
  const __v463_oldFinance = typeof finance==='function' ? finance : null;
  if(__v463_oldFinance){ finance=function(){ try{ return v463Finance(); }catch(e){ console.warn('V46.3 finance render failed', e); return __v463_oldFinance.apply(this, arguments); } }; }
...
}catch(e){ console.warn('V46.3 Finance UI Polish failed', e); }

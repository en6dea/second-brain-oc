...
function defaults(){const t=today();return {
 settings:{name:'Пользователь',subtitle:'Личный ритм',folderNames:{},hiddenSections:[],habitRange:28,planningFolder:'focus',financePeriod:'month',currentBalance:0,lastCsvAdded:0,lastCsvDuplicates:0},
 operations:[], debts:[], tasks:[], purchases:[], wishes:[], notes:[], ideas:[], people:[], habits:[], goals:[], documents:[], books:[], films:[], trips:[], personal:[], archive:[], folders:[]
}}
const DATA_COLLECTIONS=['operations','debts','tasks','purchases','wishes','notes','ideas','people','habits','goals','documents','books','films','trips','personal','archive','folders'];
function initialState(){const fresh=defaults();DATA_COLLECTIONS.forEach(k=>{fresh[k]=[]});return fresh}
...
function normalize(raw){const b=defaults();const s=raw&&typeof raw==='object'?raw:{};const m={...b,...s,settings:{...b.settings,...(s.settings||{})}};
 DATA_COLLECTIONS.forEach(k=>{m[k]=Array.isArray(s[k])?s[k]:b[k]});
...
let state=normalize(storedState??initialState());
...
function purchaseCard(x,type='purchase'){const image=safeImageUrl(x.image),url=safeUrl(x.url);return `<article class="record-card"><div class="row" style="border:0;background:transparent;padding:0"><div class="thumb">${image?`<img src="${esc(image)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:(type==='wish'?'💗':'🛒')}</div><div><h3>${esc(x.title)}</h3><p class="small muted">${fmt(x.date)} · ${esc(x.area||'Личное')}</p></div><span class="pill blue">${money(x.amount)}</span></div><p class="small muted">${esc(x.note||'')}</p><div class="row-actions"><span class="pill ${x.includeInBudget!==false?'green':'red'}">${x.includeInBudget!==false?'В бюджете':'Вне бюджета'}</span>${url?`<a class="mini blue" href="${esc(url)}" target="_blank" rel="noopener noreferrer">URL</a>`:''}<button class="mini blue" data-action="editRecord" data-type="${type}" data-id="${esc(x.id)}">Ред.</button><button class="mini red" data-action="deleteRecord" data-type="${type}" data-id="${esc(x.id)}">Удалить</button></div></article>`}
...
function recordCard(type,x){const title=x.title||x.name||x.person||'Запись';const sub=x.author||x.role||x.area||x.folder||x.place||x.type||'';const amount=x.amount||x.budget||x.target;const image=safeImageUrl(x.image),url=safeUrl(x.url);return `<article class="record-card">${image?`<img src="${esc(image)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:''}<div class="card-head"><div><h3>${esc(title)}</h3><p class="small muted">${esc(sub)}</p></div>${amount?`<span class="pill blue">${money(amount)}</span>`:''}</div><p class="small muted" style="white-space:pre-line">${esc(x.note||x.text||x.quotes||'')}</p>${url?`<a class="pill blue" href="${esc(url)}" target="_blank" rel="noopener noreferrer">Открыть URL</a>`:''}${x.includeInBudget!==undefined?`<span class="pill ${x.includeInBudget?'green':'red'}">${x.includeInBudget?'Учитывается в бюджете':'Не учитывается в бюджете'}</span>`:''}${rowActions(type,x.id)}</article>`}
...
function peoplePage(){return layout('Люди','Контакты, даты, подарки, темы разговоров и важные детали.',`<section class="grid cols-3">${state.people.map(p=>{const photo=safeImageUrl(p.photo),link=safeUrl(p.links);return `<article class="card"><div class="card-head"><div><h3>${esc(p.name)}</h3><p class="small muted">${esc(p.role||'Контакт')}</p></div><div class="avatar" style="width:58px;height:58px">${photo?`<img src="${esc(photo)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:esc((p.name||'?')[0])}</div></div><div class="grid cols-2"><div class="record-card"><b>День рождения</b><p class="small muted">${esc(p.birthday||'—')}</p></div><div class="record-card"><b>Контакты</b><p class="small muted">${esc([p.phone,p.email].filter(Boolean).join(' · ')||'—')}</p></div></div><div class="record-card"><b>Любит</b><p class="small muted" style="white-space:pre-line">${esc(p.likes||'—')}</p></div><div class="record-card"><b>Темы для разговора</b><p class="small muted" style="white-space:pre-line">${esc(p.talkIdeas||'—')}</p></div><div class="record-card"><b>Идеи подарков</b><p class="small muted" style="white-space:pre-line">${esc(p.gifts||'—')}</p></div><div class="record-card"><b>Заметки</b><p class="small muted" style="white-space:pre-line">${esc(p.notes||'—')}</p></div><div class="row-actions">${link?`<a class="mini blue" href="${esc(link)}" target="_blank" rel="noopener noreferrer">Ссылка</a>`:''}<button class="mini blue" data-action="editRecord" data-type="person" data-id="${esc(p.id)}">Ред.</button><button class="mini red" data-action="deleteRecord" data-type="person" data-id="${esc(p.id)}">Удалить</button></div></article>`}).join('')||empty('Люди пока не добавлены')}</section><div style="margin-top:16px"><button class="btn" data-action="openRecordForm" data-type="person">＋ Добавить человека</button></div>`)}
...
function exportData(){const blob=new Blob([JSON.stringify(exportableState(),null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`second-brain-os-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500)}
...
(function(){
  ...
  function v59Backup(){
    try{const payload={version:BUILD,exportedAt:new Date().toISOString(),state:exportableState()}; const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='second-brain-os-backup-'+today()+'.json'; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},500); v59Toast('Backup скачан')}catch(e){console.error(e); v59Toast('Не удалось скачать backup')}
  }
  function v59DayPlan(){
    const sig=v59TodaySignal();
...
/* ===== V61 SMART DAILY OS: smart day cockpit + action engine ===== */
(function(){
  const V61_BUILD='second-brain-space-v61-smart-daily-os-20260708';
  const V61_LABEL='V61 · SMART DAILY OS';
  try{localStorage.setItem('secondBrainOS.currentBuild',V61_BUILD);}catch(e){}

  try{
    if(Array.isArray(SECTIONS) && !SECTIONS.some(s=>s.id==='daily')){
      const idx=SECTIONS.findIndex(s=>s.id==='dashboard');
      SECTIONS.splice(idx>=0?idx+1:1,0,{id:'daily',label:'День',icon:'☀️',color:'#2563eb',group:'ПРОСТРАНСТВО'});
    }
  }catch(e){}

  function v61Arr(k){return Array.isArray(state&&state[k])?state[k]:[]}
  function v61Today(){try{return today()}catch(e){return new Date().toISOString().slice(0,10)}}
  function v61Add(d,n){try{return iso(addDays(new Date(d),n))}catch(e){const x=new Date(d);x.setDate(x.getDate()+n);return x.toISOString().slice(0,10)}}
  function v61Done(t){return ['готово','сделано','done','closed'].includes(String(t.status||'').toLowerCase())}
  function v61Esc(x){try{return esc(x)}catch(e){return String(x??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}}
  function v61Money(v){try{return money(v)}catch(e){return `${Math.round(Number(v)||0).toLocaleString('ru-RU')} ₽`}}
  function v61Fmt(d){try{return fmt(d)}catch(e){return d||'—'}}
  function v61Uid(){try{return uid()}catch(e){return 'v61_'+Date.now().toString(36)+Math.random().toString(36).slice(2)}}
  function v61Total(arr){return arr.reduce((s,x)=>s+Number(x.amount||0),0)}
  function v61SetBuild(){try{document.querySelector('meta[name="second-brain-build"]')?.setAttribute('content',V61_BUILD);document.body?.setAttribute('data-sbos-build',V61_BUILD);const v=document.querySelector('.v59-version,.version');if(v)v.textContent=V61_LABEL;}catch(e){}}

  function v61Styles(){
    if(document.getElementById('v61-smart-daily-style')) return;
    const st=document.createElement('style');
    st.id='v61-smart-daily-style';
    st.textContent=`
      .v61-page{max-width:1600px;margin:0 auto}.v61-hero{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-bottom:18px}.v61-hero h1{margin:0;font-size:38px;line-height:1;letter-spacing:-.065em;color:#061f4a}.v61-hero p{margin:9px 0 0;color:#52647f;font-weight:780}.v61-pill{display:inline-flex;align-items:center;gap:8px;border:1px solid #dbe7f6;background:rgba(255,255,255,.9);border-radius:999px;padding:10px 13px;font-weight:1000;color:#334155;box-shadow:0 14px 32px rgba(37,99,235,.07)}
      .v61-command{position:relative;overflow:hidden;border:1px solid #dbe7f6;border-radius:34px;background:linear-gradient(135deg,rgba(255,255,255,.99),rgba(239,247,255,.96));box-shadow:0 30px 76px rgba(37,99,235,.12);padding:24px;margin-bottom:18px;display:grid;grid-template-columns:minmax(0,1.4fr) minmax(360px,.9fr);gap:18px}.v61-command:before{content:'';position:absolute;right:-120px;top:-130px;width:340px;height:340px;border-radius:50%;background:radial-gradient(circle,rgba(37,99,235,.16),transparent 68%)}.v61-command>*{position:relative;z-index:1}.v61-command h2{margin:0;font-size:28px;letter-spacing:-.06em;color:#102044}.v61-command p{margin:10px 0 0;color:#64748b;font-weight:820;line-height:1.48}.v61-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px}.v61-actions button{border:1px solid #dbe7f6;background:#fff;border-radius:16px;padding:12px 14px;font-weight:950;color:#334155;box-shadow:0 12px 26px rgba(37,99,235,.05)}.v61-actions button.primary{border:0;color:#fff;background:linear-gradient(135deg,#2563eb,#38bdf8);box-shadow:0 20px 38px rgba(37,99,235,.21)}.v61-actions button.green{border:0;color:#fff;background:linear-gradient(135deg,#10b981,#34d399)}
      .v61-grid{display:grid;gap:16px}.v61-cols-2{grid-template-columns:1.13fr .87fr}.v61-cols-3{grid-template-columns:repeat(3,minmax(0,1fr))}.v61-cols-4{grid-template-columns:repeat(4,minmax(0,1fr))}.v61-card,.v61-stat,.v61-signal,.v61-action-card{position:relative;overflow:hidden;border:1px solid #dbe7f6;border-radius:28px;background:linear-gradient(180deg,rgba(255,255,255,.98),rgba(248,251,255,.98));box-shadow:0 22px 56px rgba(37,99,235,.085);padding:18px}.v61-card:before,.v61-stat:before,.v61-action-card:before{content:'';position:absolute;right:-60px;top:-70px;width:190px;height:190px;border-radius:50%;background:radial-gradient(circle,rgba(37,99,235,.08),transparent 70%);pointer-events:none}.v61-card>* , .v61-stat>* , .v61-action-card>*{position:relative;z-index:1}.v61-card h3,.v61-action-card h3{margin:0;font-size:19px;letter-spacing:-.04em;color:#102044}.v61-card p,.v61-action-card p{margin:8px 0 0;color:#64748b;font-size:13px;font-weight:820;line-height:1.45}.v61-stat span{display:block;font-size:11px;letter-spacing:.09em;text-transform:uppercase;color:#7b8aa4;font-weight:1000}.v61-stat b{display:block;margin-top:9px;font-size:32px;letter-spacing:-.06em;color:#102044}.v61-stat b.blue{color:#2563eb}.v61-stat b.green{color:#10b981}.v61-stat b.red{color:#ef4444}.v61-stat b.amber{color:#f59e0b}.v61-stat small{display:block;margin-top:6px;color:#64748b;font-size:12px;font-weight:820;line-height:1.35}
      .v61-radar{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.v61-radar-item{border:1px solid #e2ebf7;background:#fff;border-radius:20px;padding:13px}.v61-radar-item span{font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:#7b8aa4;font-weight:1000}.v61-radar-item b{display:block;margin-top:6px;font-size:20px;color:#102044;letter-spacing:-.04em}.v61-radar-item small{display:block;margin-top:3px;color:#64748b;font-size:12px;font-weight:820}.v61-score{display:grid;place-items:center;min-height:160px;border:1px solid #dbe7f6;border-radius:28px;background:radial-gradient(circle at 50% 28%,rgba(37,99,235,.12),transparent 44%),linear-gradient(180deg,#fff,#f8fbff)}.v61-score strong{font-size:52px;line-height:1;letter-spacing:-.08em;color:#2563eb}.v61-score span{display:block;margin-top:8px;color:#64748b;font-weight:900}.v61-score.good strong{color:#10b981}.v61-score.warn strong{color:#f59e0b}.v61-score.danger strong{color:#ef4444}
      .v61-plan-list{display:grid;gap:12px}.v61-plan-row{display:grid;grid-template-columns:44px minmax(0,1fr) auto;gap:12px;align-items:center;border:1px solid #e2ebf7;background:#fff;border-radius:22px;padding:14px}.v61-plan-ico{width:44px;height:44px;border-radius:16px;display:grid;place-items:center;background:#eef5ff;color:#2563eb;font-weight:1000}.v61-plan-row b{display:block;color:#102044;letter-spacing:-.025em}.v61-plan-row small{display:block;margin-top:3px;color:#64748b;font-weight:820;line-height:1.35}.v61-mini-btn{border:1px solid #dbe7f6;background:#f8fbff;color:#2563eb;border-radius:13px;padding:9px 11px;font-weight:950}.v61-mini-btn:hover{background:#eef5ff}.v61-badge{display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:7px 10px;font-size:12px;font-weight:1000;border:1px solid #dbe7f6;background:#f8fbff;color:#475569}.v61-badge.blue{background:#eef5ff;color:#2563eb;border-color:#bfdbfe}.v61-badge.green{background:#ecfdf5;color:#10b981;border-color:#bbf7d0}.v61-badge.red{background:#fff1f2;color:#ef4444;border-color:#fecaca}.v61-badge.amber{background:#fff7ed;color:#d97706;border-color:#fed7aa}
      .v61-timeline{display:grid;gap:10px}.v61-time-row{display:grid;grid-template-columns:76px minmax(0,1fr) auto;gap:12px;align-items:center;border:1px solid #e2ebf7;border-radius:20px;background:#fff;padding:12px}.v61-time-row time{font-weight:1000;color:#2563eb}.v61-time-row b{color:#102044}.v61-time-row small{display:block;color:#64748b;font-weight:820;margin-top:2px}.v61-ai{border:1px solid #dbe7f6;border-radius:28px;background:linear-gradient(135deg,#102044,#2563eb);color:#fff;padding:20px;box-shadow:0 26px 62px rgba(37,99,235,.22)}.v61-ai h3{margin:0;font-size:21px;letter-spacing:-.045em}.v61-ai p{color:rgba(255,255,255,.80);font-weight:820;line-height:1.48}.v61-ai .v61-actions button{background:rgba(255,255,255,.14);border-color:rgba(255,255,255,.22);color:#fff}.v61-ai .v61-actions button.primary{background:#fff;color:#2563eb}.v61-note{border:1px dashed #c7dbf7;background:#f8fbff;border-radius:20px;padding:14px;color:#53657f;font-weight:830;font-size:13px;line-height:1.5}.v61-nav-spark{box-shadow:0 0 0 4px rgba(37,99,235,.08)!important}
      @media(max-width:1280px){.v61-command,.v61-cols-2{grid-template-columns:1fr}.v61-cols-4{grid-template-columns:repeat(2,minmax(0,1fr))}.v61-radar{grid-template-columns:repeat(4,minmax(0,1fr))}}
      @media(max-width:760px){.v61-hero{align-items:flex-start;flex-direction:column}.v61-hero h1{font-size:30px}.v61-command{padding:18px;border-radius:26px}.v61-cols-3,.v61-cols-4,.v61-radar{grid-template-columns:1fr}.v61-plan-row,.v61-time-row{grid-template-columns:1fr}.v61-actions button{width:100%;justify-content:center}.v61-page{padding-bottom:80px}}
    `;
    document.head.appendChild(st);
  }

  function v61Health(){
    const d=v61Today(), week=v61Add(d,7);
    const tasks=v61Arr('tasks');
    const activeTasks=tasks.filter(t=>!v61Done(t));
    const todayTasks=activeTasks.filter(t=>(t.date||d)===d);
    const overdueTasks=activeTasks.filter(t=>t.date && t.date<d);
    const weekTasks=activeTasks.filter(t=>t.date && t.date>d && t.date<=week);
    const debts=(typeof activeDebts==='function'?activeDebts():v61Arr('debts').filter(x=>String(x.status||'').toLowerCase()!=='закрыт'));
    const dueOut=debts.filter(x=>x.direction==='out' && x.due && x.due<=week);
    const dueIn=debts.filter(x=>x.direction==='in' && x.due && x.due<=week);
    const overdueDebts=debts.filter(x=>x.due && x.due<d);
    const habits=v61Arr('habits');
    const habitsDone=habits.filter(h=>h.marks&&h.marks[d]).length;
    const f=(typeof periodInfo==='function'&&typeof financeTotals==='function')?financeTotals(periodInfo('month')):{inc:0,exp:0,net:0};
    const load=todayTasks.length + overdueTasks.length*2 + dueOut.length*2 + overdueDebts.length*2;
    const score=Math.max(0,Math.min(100,100-load*7));
    return {d,week,tasks,activeTasks,todayTasks,overdueTasks,weekTasks,debts,dueOut,dueIn,overdueDebts,habits,habitsDone,f,score,load};
  }

  function v61FocusTask(h){
    const priority={A:1,'Высокий':1,B:2,'Средний':2,C:3,'Низкий':3};
    return [...h.overdueTasks,...h.todayTasks,...h.weekTasks].sort((a,b)=>(priority[a.priority]||9)-(priority[b.priority]||9)||String(a.date||'').localeCompare(String(b.date||'')))[0];
  }

  function v61PlanRows(h){
    const focus=v61FocusTask(h);
    const debt=h.dueOut.sort((a,b)=>String(a.due||'').localeCompare(String(b.due||'')))[0];
    const income=h.dueIn.sort((a,b)=>String(a.due||'').localeCompare(String(b.due||'')))[0];
    const rows=[];
    if(focus) rows.push(['✅','Главный шаг',focus.title,`${focus.area||'Задача'} · ${focus.date?v61Fmt(focus.date):'без даты'} · ${focus.time||'без времени'}`,'tasks','Открыть']);
    else rows.push(['✅','Главный шаг','Создать один маленький шаг на 15 минут','Нет задачи, которую система может назначить главной','tasks','Создать']);
    if(debt) rows.push(['⚖️','Долг / обязательство',`${debt.person} · ${v61Money(debt.amount)}`,`срок: ${v61Fmt(debt.due)} · лучше не тянуть`,'debts','Долги']);
    else if(income) rows.push(['↗','Возврат денег',`${income.person} · ${v61Money(income.amount)}`,`ожидается до ${v61Fmt(income.due)}`,'debts','Проверить']);
    else rows.push(['💸','Финансовый контроль','Проверить лимит дня и расходы','долгов на 7 дней не найдено','finance','Финансы']);
    if(h.habits.length) rows.push(['🎯','Минимум ритма',`Отметить ${Math.max(0,Math.min(3,h.habits.length)-h.habitsDone)} из ${h.habits.length} привычек`,`сегодня отмечено ${h.habitsDone}/${h.habits.length}`,'habits','Привычки']);
    rows.push(['📥','Разгрузка головы','Скинуть всё лишнее во входящие','чтобы не держать в голове и не перегружаться','inbox','Входящие']);
    return rows.slice(0,4);
  }

  function v61LoadLabel(h){
    if(h.score>=75) return ['good','Лёгкий день','Можно спокойно закрывать главное'];
    if(h.score>=45) return ['warn','Средняя нагрузка','Важно не распыляться'];
    return ['danger','Перегруз','Нужен режим минимума'];
  }

  function v61DailyPage(){
    v61Styles();
    const h=v61Health();
    const [scoreCls,loadTitle,loadSub]=v61LoadLabel(h);
    const plan=v61PlanRows(h);
    const todayTimeline=[...h.todayTasks].sort((a,b)=>String(a.time||'99:99').localeCompare(String(b.time||'99:99'))).slice(0,8);
    const debtAmount=v61Total(h.dueOut);
    const incomingAmount=v61Total(h.dueIn);
    const balance=Number(state.settings?.currentBalance||0);
    const dayBudget=Math.max(0,Math.round((balance+incomingAmount-debtAmount)/Math.max(1,7)));
    return `<div class="v61-page"><div class="v61-hero"><div><h1>Умный день</h1><p>Операционный экран: что важно сегодня, где риск и какой следующий шаг сделать без перегруза.</p></div><div class="v61-pill">☷ ${new Date().toLocaleDateString('ru-RU',{day:'numeric',month:'long'})}</div></div>
      <section class="v61-command"><div><span class="v61-badge blue">Smart Daily OS</span><h2>Сегодня: ${loadTitle}</h2><p>${loadSub}. Система собрала задачи, долги, финансы и привычки в один короткий план, чтобы не прыгать по разделам.</p><div class="v61-actions"><button class="primary" data-v61-action="createTinyTask">Создать шаг 15 минут</button><button class="green" data-go="tasks">Открыть задачи</button><button data-go="finance">Финансы</button><button data-go="habits">Привычки</button></div></div><div class="v61-score ${scoreCls}"><strong>${h.score}%</strong><span>${loadTitle}</span></div></section>
      <section class="v61-grid v61-cols-4" style="margin-bottom:16px"><article class="v61-stat"><span>Сегодня</span><b class="blue">${h.todayTasks.length}</b><small>активных задач на день</small></article><article class="v61-stat"><span>Просрочено</span><b class="${h.overdueTasks.length?'red':'green'}">${h.overdueTasks.length}</b><small>задач требуют решения</small></article><article class="v61-stat"><span>Долги 7 дней</span><b class="${debtAmount?'red':'green'}">${v61Money(debtAmount)}</b><small>${h.dueOut.length} обязательств рядом</small></article><article class="v61-stat"><span>Ритм привычек</span><b class="green">${h.habitsDone}/${h.habits.length}</b><small>отмечено сегодня</small></article></section>
      <section class="v61-grid v61-cols-2"><article class="v61-card"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:14px"><div><h3>Что делать сейчас</h3><p>Короткий список вместо перегруза и бесконечного выбора.</p></div><span class="v61-badge green">активный план</span></div><div class="v61-plan-list">${plan.map(r=>`<div class="v61-plan-row"><span class="v61-plan-ico">${r[0]}</span><div><b>${v61Esc(r[1])}: ${v61Esc(r[2])}</b><small>${v61Esc(r[3])}</small></div><button class="v61-mini-btn" data-go="${r[4]}">${v61Esc(r[5])}</button></div>`).join('')}</div></article>
      <aside class="v61-card"><h3>Радар дня</h3><p>Быстрый снимок нагрузки и денег.</p><div class="v61-radar" style="margin-top:14px"><div class="v61-radar-item"><span>Баланс</span><b>${v61Money(balance)}</b><small>факт вручную</small></div><div class="v61-radar-item"><span>Лимит 7 дней</span><b>${v61Money(dayBudget)}</b><small>оценка после долгов</small></div><div class="v61-radar-item"><span>Ожидаемые возвраты</span><b>${v61Money(incomingAmount)}</b><small>${h.dueIn.length} поступлений рядом</small></div><div class="v61-radar-item"><span>Месяц</span><b class="${h.f.net<0?'red':'green'}">${v61Money(h.f.net||0)}</b><small>прогноз остатка</small></div></div><div class="v61-note" style="margin-top:14px">Если день перегружен — не добавляй новые задачи. Закрой один главный шаг, один финансовый риск и минимум привычек.</div></aside></section>
      <section class="v61-grid v61-cols-2" style="margin-top:16px"><article class="v61-card"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:14px"><div><h3>Календарная лента</h3><p>То, что стоит на сегодня по времени.</p></div><button class="v61-mini-btn" data-go="calendar">Календарь</button></div><div class="v61-timeline">${todayTimeline.map(t=>`<div class="v61-time-row"><time>${v61Esc(t.time||'—')}</time><div><b>${v61Esc(t.title)}</b><small>${v61Esc(t.area||'Задача')} · ${v61Esc(t.priority||'B')}</small></div><span class="v61-badge blue">${v61Esc(t.status||'активно')}</span></div>`).join('')||'<div class="v61-note">На сегодня нет задач по времени. Можно выбрать один шаг вручную.</div>'}</div></article><article class="v61-ai"><h3>AI-помощник дня</h3><p>Быстрые команды для следующей версии AI-логики. Сейчас они уже дают рабочие действия внутри системы.</p><div class="v61-actions"><button class="primary" data-v61-action="createTinyTask">Сделать шаг 15 минут</button><button data-v61-action="createReview">Создать обзор дня</button><button data-go="attention">Центр внимания</button><button data-go="global-search">Поиск</button></div></article></section>
    </div>`;
  }

  function v61CreateTinyTask(){
    const h=v61Health(); const focus=v61FocusTask(h);
    state.tasks=Array.isArray(state.tasks)?state.tasks:[];
    state.tasks.unshift({id:v61Uid(),title:`15 минут: ${focus?focus.title:'разобрать главный завал дня'}`,area:focus?.area||'Фокус',date:v61Today(),time:'',priority:'A',status:'Активно',google:false,reminder:'',note:'Создано Smart Daily OS'});
    try{save()}catch(e){}
    try{(typeof toast==='function'?toast:alert)('Создал шаг на 15 минут')}catch(e){}
    v61Go('tasks');
  }
  function v61CreateReview(){
    state.notes=Array.isArray(state.notes)?state.notes:[];
    state.notes.unshift({id:v61Uid(),title:'Обзор дня',folder:'Обзоры',date:v61Today(),text:'Что было главным сегодня?\nЧто забрало силы?\nЧто перенести?\nКакой один шаг завтра?'});
    try{save()}catch(e){}
    try{(typeof toast==='function'?toast:alert)('Создал заметку для обзора дня')}catch(e){}
    v61Go('notes');
  }

  function v61Post(){
    try{
      v61Styles(); v61SetBuild();
      const current=(location.hash||'').replace('#','')||page||'dashboard';
      document.querySelectorAll('[data-go="daily"]').forEach(el=>el.classList.add('v61-nav-spark'));
      if(current==='daily'){
        const view=document.querySelector('#view');
        if(view) view.innerHTML=v61DailyPage();
        document.querySelectorAll('.v59-nav-item').forEach(el=>el.classList.toggle('active',el.dataset.go==='daily'));
      }
    }catch(e){console.error('[V61 post]',e)}
  }

  const oldGoV61=typeof go==='function'?go:null;
  function v61Go(id){
    if(id==='daily'){
      page='daily';
      try{location.hash='daily'}catch(e){}
      try{if(typeof render==='function') render(); else v61Post();}catch(e){v61Post();}
      setTimeout(v61Post,80);
      return;
    }
    if(oldGoV61) return oldGoV61(id);
    try{location.hash=id; if(typeof render==='function') render();}catch(e){}
  }
  try{go=v61Go;}catch(e){}

  const oldRenderV61=typeof render==='function'?render:null;
  if(oldRenderV61){
    render=function(){
      const r=oldRenderV61.apply(this,arguments);
      setTimeout(v61Post,80);
      return r;
    };
  }
  ...
  window.addEventListener('click',function(e){
    const a=e.target.closest&&e.target.closest('[data-v61-action]');
    if(!a) return;
    e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
    const act=a.dataset.v61Action;
    if(act==='createTinyTask') return v61CreateTinyTask();
    if(act==='createReview') return v61CreateReview();
  },true);
  window.addEventListener('hashchange',()=>{if((location.hash||'').replace('#','')==='daily') setTimeout(v61Post,90);});

  try{v61Styles();v61SetBuild();render();}catch(e){console.error('[V61 init]',e)}
})();


/* ===== V62 AI LIFE COACH: real coach page + action engine ===== */
(function(){
  const V62_BUILD='second-brain-space-v62-ai-life-coach-20260709';
  const V62_LABEL='V62 · AI LIFE COACH';
  try{localStorage.setItem('secondBrainOS.currentBuild',V62_BUILD);}catch(e){}

  function v62Arr(k){return Array.isArray(state&&state[k])?state[k]:[]}
  function v62Today(){return typeof today==='function'?today():new Date().toISOString().slice(0,10)}
  function v62Add(date,n){const d=new Date(date);d.setDate(d.getDate()+n);return d.toISOString().slice(0,10)}
  function v62Esc(v){return typeof esc==='function'?esc(v):String(v??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[ch]))}
  function v62Money(v){try{return money(v)}catch(e){return `${Math.round(Number(v)||0).toLocaleString('ru-RU')} ₽`}}
  function v62Fmt(d){try{return fmt(d)}catch(e){return d||'—'}}
  function v62Uid(){try{return uid()}catch(e){return 'v62_'+Date.now().toString(36)+Math.random().toString(36).slice(2)}}
  function v62Done(x){return ['готово','закрыт','закрыто','done'].includes(String(x.status||'').trim().toLowerCase())}
  function v62Sum(arr){return arr.reduce((s,x)=>s+Number(x.amount||0),0)}
  function v62Toast(msg){try{(typeof v59Toast==='function'?v59Toast:(typeof toast==='function'?toast:alert))(msg)}catch(e){}}

  function v62Ensure(){
    state.settings=state.settings||{};
    state.settings.v62=Object.assign({mode:'focus',lastBriefAt:''},state.settings.v62||{});
    if(Array.isArray(SECTIONS) && !SECTIONS.some(s=>s.id==='coach')){
      const idx=SECTIONS.findIndex(s=>s.id==='daily');
      SECTIONS.splice(idx>=0?idx+1:1,0,{id:'coach',label:'AI Coach',icon:'✦',color:'#7c3aed',group:'ПРОСТРАНСТВО'});
    }
  }

  function v62SetBuild(){
    try{
      document.querySelector('meta[name="second-brain-build"]')?.setAttribute('content',V62_BUILD);
      document.body?.setAttribute('data-sbos-build',V62_BUILD);
      const v=document.querySelector('.v59-version,.version');
      if(v) v.textContent=V62_LABEL;
    }catch(e){}
  }

  function v62Styles(){
    if(document.getElementById('v62-ai-life-coach-style')) return;
    const st=document.createElement('style');
    st.id='v62-ai-life-coach-style';
    st.textContent=`
      .v62-page{max-width:1600px;margin:0 auto;padding-bottom:34px}.v62-hero{position:relative;overflow:hidden;border:1px solid #dbe7f6;border-radius:34px;background:radial-gradient(circle at 88% 10%,rgba(124,58,237,.18),transparent 32%),linear-gradient(135deg,#ffffff,#f5f9ff);box-shadow:0 30px 76px rgba(37,99,235,.12);padding:26px;display:grid;grid-template-columns:minmax(0,1.18fr) minmax(300px,.82fr);gap:20px;margin-bottom:18px}.v62-kicker{display:inline-flex;align-items:center;gap:8px;border:1px solid #ddd6fe;background:#f5f3ff;color:#7c3aed;border-radius:999px;padding:8px 11px;font-size:12px;font-weight:1000}.v62-hero h1{margin:14px 0 8px;color:#071d44;font-size:40px;line-height:1;letter-spacing:-.065em}.v62-hero p{margin:0;color:#58708f;font-weight:820;line-height:1.55}.v62-hero-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px}.v62-btn{border:1px solid #dbe7f6;background:#fff;color:#334155;border-radius:16px;padding:12px 14px;font-weight:1000;box-shadow:0 12px 26px rgba(37,99,235,.05)}.v62-btn.primary{border:0;color:#fff;background:linear-gradient(135deg,#7c3aed,#2563eb);box-shadow:0 20px 42px rgba(124,58,237,.22)}.v62-btn.green{border:0;color:#fff;background:linear-gradient(135deg,#10b981,#34d399)}.v62-btn:hover{transform:translateY(-1px)}.v62-mode-card{border:1px solid #dbe7f6;border-radius:28px;background:rgba(255,255,255,.76);padding:16px;display:grid;gap:10px}.v62-mode-card h3{margin:0;color:#102044;letter-spacing:-.04em}.v62-mode-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.v62-mode{border:1px solid #e3ecf8;background:#fff;border-radius:18px;padding:12px;text-align:left;font-weight:1000;color:#334155}.v62-mode small{display:block;margin-top:5px;color:#64748b;font-weight:780;line-height:1.35}.v62-mode.active{border-color:#c4b5fd;background:linear-gradient(180deg,#f5f3ff,#fff);color:#6d28d9;box-shadow:0 14px 30px rgba(124,58,237,.12)}
      .v62-grid{display:grid;gap:16px}.v62-cols-2{grid-template-columns:1.08fr .92fr}.v62-cols-3{grid-template-columns:repeat(3,minmax(0,1fr))}.v62-card,.v62-signal,.v62-command-card{position:relative;overflow:hidden;border:1px solid #dbe7f6;border-radius:28px;background:linear-gradient(180deg,rgba(255,255,255,.98),rgba(248,251,255,.98));box-shadow:0 22px 56px rgba(37,99,235,.085);padding:18px}.v62-card:before,.v62-command-card:before{content:'';position:absolute;right:-72px;top:-82px;width:220px;height:220px;border-radius:50%;background:radial-gradient(circle,rgba(124,58,237,.08),transparent 68%);pointer-events:none}.v62-card>* , .v62-command-card>*{position:relative;z-index:1}.v62-card h3,.v62-command-card h3{margin:0;color:#102044;font-size:20px;letter-spacing:-.045em}.v62-card p,.v62-command-card p{margin:8px 0 0;color:#64748b;font-size:13px;font-weight:820;line-height:1.48}.v62-signal span{display:block;font-size:11px;letter-spacing:.09em;text-transform:uppercase;color:#7b8aa4;font-weight:1000}.v62-signal b{display:block;margin-top:9px;font-size:32px;letter-spacing:-.06em;color:#102044}.v62-signal small{display:block;margin-top:6px;color:#64748b;font-size:12px;font-weight:820;line-height:1.35}.v62-blue{color:#2563eb!important}.v62-green{color:#10b981!important}.v62-red{color:#ef4444!important}.v62-amber{color:#f59e0b!important}.v62-violet{color:#7c3aed!important}
      .v62-brief{display:grid;gap:12px}.v62-brief-row{display:grid;grid-template-columns:48px minmax(0,1fr) auto;gap:12px;align-items:center;border:1px solid #e2ebf7;background:#fff;border-radius:22px;padding:14px}.v62-ico{width:48px;height:48px;border-radius:18px;display:grid;place-items:center;background:#f5f3ff;color:#7c3aed;font-weight:1000}.v62-brief-row b{display:block;color:#102044;letter-spacing:-.025em}.v62-brief-row small{display:block;margin-top:4px;color:#64748b;font-weight:820;line-height:1.35}.v62-badge{display:inline-flex;align-items:center;border:1px solid #dbe7f6;background:#f8fbff;border-radius:999px;padding:7px 10px;font-size:12px;font-weight:1000;color:#475569;white-space:nowrap}.v62-badge.blue{background:#eef5ff;color:#2563eb;border-color:#bfdbfe}.v62-badge.green{background:#ecfdf5;color:#10b981;border-color:#bbf7d0}.v62-badge.red{background:#fff1f2;color:#ef4444;border-color:#fecaca}.v62-badge.amber{background:#fff7ed;color:#d97706;border-color:#fed7aa}.v62-badge.violet{background:#f5f3ff;color:#7c3aed;border-color:#ddd6fe}
      .v62-command-list{display:grid;gap:11px}.v62-command-card{display:grid;grid-template-columns:54px minmax(0,1fr);gap:14px;align-items:start}.v62-command-card .v62-ico{width:54px;height:54px}.v62-command-card .v62-actions{grid-column:1/-1;display:flex;gap:9px;flex-wrap:wrap;margin-top:2px}.v62-mini{border:1px solid #dbe7f6;background:#fff;color:#334155;border-radius:13px;padding:9px 11px;font-size:12px;font-weight:1000}.v62-mini.primary{background:#eef5ff;color:#2563eb;border-color:#bfdbfe}.v62-mini.green{background:#ecfdf5;color:#10b981;border-color:#bbf7d0}.v62-mini.red{background:#fff1f2;color:#ef4444;border-color:#fecaca}.v62-insight{border:1px solid #dbe7f6;background:linear-gradient(135deg,#102044,#2563eb);color:#fff;border-radius:30px;padding:20px;box-shadow:0 28px 66px rgba(37,99,235,.24)}.v62-insight h3{margin:0;font-size:22px;letter-spacing:-.045em}.v62-insight p{color:rgba(255,255,255,.82);font-weight:820;line-height:1.55}.v62-insight .v62-mini{background:rgba(255,255,255,.14);border-color:rgba(255,255,255,.24);color:#fff}.v62-insight .v62-mini.primary{background:#fff;color:#2563eb}.v62-note{border:1px dashed #c7dbf7;background:#f8fbff;border-radius:20px;padding:14px;color:#53657f;font-size:13px;font-weight:830;line-height:1.5}.v62-nav-hot{box-shadow:0 0 0 4px rgba(124,58,237,.10)!important}
      @media(max-width:1280px){.v62-hero,.v62-cols-2{grid-template-columns:1fr}.v62-cols-3{grid-template-columns:1fr 1fr}}@media(max-width:760px){.v62-hero{padding:18px;border-radius:26px}.v62-hero h1{font-size:31px}.v62-cols-3,.v62-mode-grid{grid-template-columns:1fr}.v62-brief-row,.v62-command-card{grid-template-columns:1fr}.v62-hero-actions .v62-btn,.v62-command-card .v62-mini{width:100%;justify-content:center}.v62-page{padding-bottom:86px}}
    `;
    document.head.appendChild(st);
  }

  function v62Health(){
    const d=v62Today(), week=v62Add(d,7);
    const tasks=v62Arr('tasks');
    const activeTasks=tasks.filter(t=>!v62Done(t));
    const todayTasks=activeTasks.filter(t=>(t.date||d)===d);
    const overdueTasks=activeTasks.filter(t=>t.date && t.date<d);
    const weekTasks=activeTasks.filter(t=>t.date && t.date>d && t.date<=week);
    const debts=(typeof activeDebts==='function'?activeDebts():v62Arr('debts').filter(x=>!v62Done(x)));
    ...
    const overdueDebts=debts.filter(x=>x.due && x.due<d);
    const habits=v62Arr('habits');
    const habitsDone=habits.filter(h=>h.marks&&h.marks[d]).length;
    const openHabits=habits.filter(h=>!(h.marks&&h.marks[d]));
    const f=(typeof periodInfo==='function'&&typeof financeTotals==='function')?financeTotals(periodInfo('month')):{inc:0,exp:0,net:0};
    const load=todayTasks.length + overdueTasks.length*2 + dueOut.length*2 + overdueDebts.length*2;
    const score=Math.max(0,Math.min(100,100-load*7));
    const focus=[...overdueTasks,...todayTasks,...weekTasks].sort((a,b)=>{
      const p={A:1,'Высокий':1,B:2,'Средний':2,C:3,'Низкий':3};
      return (p[a.priority]||9)-(p[b.priority]||9)||String(a.date||'').localeCompare(String(b.date||''));
    })[0];
    return {d,week,tasks,activeTasks,todayTasks,overdueTasks,weekTasks,debts,dueOut,dueIn,overdueDebts,habits,habitsDone,openHabits,f,load,score,focus};
  }

  function v62ModeInfo(mode,h){
    if(mode==='minimum') return ['Режим минимума','Закрыть один важный шаг, один финансовый риск и минимум ритма. Остальное не трогать.','amber'];
    if(mode==='money') return ['Финансовый режим','Сначала долги, возвраты и лимит дня. Задачи без денег можно отложить.','green'];
    return h.score<45?['Фокус без перегруза','День перегружен: система предлагает короткий план без распыления.','red']:['Фокус дня','Выбрать главное действие и спокойно вести день по короткому маршруту.','blue'];
  }

  function v62BriefRows(h){
    const debt=h.dueOut[0];
    const incoming=h.dueIn[0];
    const habit=h.openHabits[0];
    const rows=[];
    if(h.focus) rows.push(['✓','Главный шаг',h.focus.title,`${h.focus.area||'Задача'} · ${h.focus.date?v62Fmt(h.focus.date):'без даты'} ${h.focus.time?'· '+h.focus.time:''}`,'tasks','blue']);
    else rows.push(['✓','Главный шаг','Создать маленькую задачу на 15 минут','Сейчас нет очевидной главной задачи','tasks','green']);
    if(debt) rows.push(['₽','Финансовый риск',`${debt.person} · ${v62Money(debt.amount)}`,`срок ${v62Fmt(debt.due)} · лучше поставить напоминание`,'debts','red']);
    else if(incoming) rows.push(['↗','Возврат денег',`${incoming.person} · ${v62Money(incoming.amount)}`,`ожидается до ${v62Fmt(incoming.due)}`,'debts','green']);
    else rows.push(['₽','Деньги','Проверить лимит дня и свежие расходы',`прогноз месяца: ${v62Money(h.f.net||0)}`,'finance','blue']);
    if(h.habits.length) rows.push(['∿','Ритм',habit?`Отметить: ${habit.name||habit.title||'привычку'}`:'Ритм на сегодня закрыт',`${h.habitsDone}/${h.habits.length} привычек отмечено сегодня`,'habits',habit?'amber':'green']);
    rows.push(['☷','Разгрузка','Сбросить лишнее во входящие','чтобы не держать в голове и не прыгать между папками','inbox','violet']);
    return rows;
  }

  function v62BriefText(h){
    const mode=state.settings?.v62?.mode||'focus';
    const [title,sub]=v62ModeInfo(mode,h);
    const debt=h.dueOut[0];
    const habit=h.openHabits[0];
    return [
      `AI Life Coach · ${title}`,
      `Дата: ${v62Fmt(h.d)}`,
      '',
      `Состояние дня: ${h.score}% · задач сегодня ${h.todayTasks.length} · просрочено ${h.overdueTasks.length}.`,
      `Главное действие: ${h.focus?h.focus.title:'создать один шаг на 15 минут'}.`,
      `Финансовый фокус: ${debt?`${debt.person}, ${v62Money(debt.amount)}, срок ${v62Fmt(debt.due)}`:`прогноз месяца ${v62Money(h.f.net||0)}`}.`,
      `Ритм: ${h.habitsDone}/${h.habits.length}${habit?`, следующий минимум — ${habit.name||habit.title||'привычка'}`:''}.`,
      '',
      `Рекомендация: ${sub}`
    ].join('\n');
  }

  function v62CoachPage(){
    v62Ensure(); v62Styles();
    const h=v62Health();
    const mode=state.settings?.v62?.mode||'focus';
    const [modeTitle,modeSub,modeColor]=v62ModeInfo(mode,h);
    const debtAmount=v62Sum(h.dueOut);
    const incoming=v62Sum(h.dueIn);
    const balance=Number(state.settings?.currentBalance||0);
    const safeLimit=Math.max(0,Math.round((balance+incoming-debtAmount)/7));
    const rows=v62BriefRows(h);
    return `<div class="v62-page">
      <section class="v62-hero">
        <div><span class="v62-kicker">✦ AI Life Coach</span><h1>${v62Esc(modeTitle)}</h1><p>${v62Esc(modeSub)} Помощник работает без внешнего API: он читает текущие задачи, долги, финансы и привычки, а потом создаёт конкретные действия внутри приложения.</p><div class="v62-hero-actions"><button class="v62-btn primary" data-v62-action="createMinimumDay">Собрать день</button><button class="v62-btn green" data-v62-action="createCoachBrief">Создать brief</button><button class="v62-btn" data-go="daily">Умный день</button><button class="v62-btn" data-go="system">Система</button></div></div>
        <aside class="v62-mode-card"><h3>Режим помощника</h3><div class="v62-mode-grid">${[['focus','Фокус','главное действие'],['minimum','Минимум','когда перегруз'],['money','Деньги','долги и лимит']].map(([id,t,s])=>`<button class="v62-mode ${mode===id?'active':''}" data-v62-action="setMode" data-mode="${id}">${t}<small>${s}</small></button>`).join('')}</div><div class="v62-note">Текущий режим влияет на рекомендации и создаваемые действия. Это уже рабочая логика, не декоративная карточка.</div></aside>
      </section>
      <section class="v62-grid v62-cols-3" style="margin-bottom:16px">
        <article class="v62-signal"><span>Нагрузка дня</span><b class="v62-${modeColor}">${h.score}%</b><small>${h.score<45?'режим минимума':'можно вести день спокойно'}</small></article>
        <article class="v62-signal"><span>Финансовый риск</span><b class="${debtAmount?'v62-red':'v62-green'}">${v62Money(debtAmount)}</b><small>${h.dueOut.length} обязательств на 7 дней</small></article>
        <article class="v62-signal"><span>Лимит 7 дней</span><b class="v62-blue">${v62Money(safeLimit)}</b><small>оценка после ближайших долгов</small></article>
      </section>
      <section class="v62-grid v62-cols-2">
        <article class="v62-card"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:14px"><div><h3>Короткий brain-brief</h3><p>Самое важное из всей системы в одном месте.</p></div><button class="v62-mini primary" data-v62-action="copyBrief">Скопировать</button></div><div class="v62-brief">${rows.map(r=>`<div class="v62-brief-row"><span class="v62-ico">${r[0]}</span><div><b>${v62Esc(r[1])}: ${v62Esc(r[2])}</b><small>${v62Esc(r[3])}</small></div><button class="v62-mini ${r[5]==='red'?'red':r[5]==='green'?'green':'primary'}" data-go="${r[4]}">Открыть</button></div>`).join('')}</div></article>
        <aside class="v62-insight"><h3>Что бы я сделал сейчас</h3><p>${v62Esc(v62BriefText(h).split('\n').slice(4).join(' '))}</p><div class="v62-hero-actions"><button class="v62-mini primary" data-v62-action="createTinyTask">Шаг 15 минут</button><button class="v62-mini" data-v62-action="createDebtTask">Фин. напоминание</button><button class="v62-mini" data-v62-action="createHabitTask">Минимум привычки</button></div></aside>
      </section>
      <section class="v62-grid v62-cols-3" style="margin-top:16px">
        <article class="v62-command-card"><span class="v62-ico">1</span><div><h3>Разобрать перегруз</h3><p>Создаёт один маленький шаг вместо большого комка задач.</p></div><div class="v62-actions"><button class="v62-mini primary" data-v62-action="createTinyTask">Создать задачу</button><button class="v62-mini" data-go="tasks">Открыть задачи</button></div></article>
        <article class="v62-command-card"><span class="v62-ico">2</span><div><h3>Закрыть денежный риск</h3><p>Берёт ближайший долг/обязательство и делает напоминание на сегодня.</p></div><div class="v62-actions"><button class="v62-mini red" data-v62-action="createDebtTask">Создать напоминание</button><button class="v62-mini" data-go="debts">Долги</button></div></article>
        <article class="v62-command-card"><span class="v62-ico">3</span><div><h3>Сохранить ритм</h3><p>Если привычки просели, создаёт минимальную задачу без чувства провала.</p></div><div class="v62-actions"><button class="v62-mini green" data-v62-action="createHabitTask">Минимум ритма</button><button class="v62-mini" data-go="habits">Привычки</button></div></article>
      </section>
    </div>`;
  }

  function v62CreateTinyTask(){
    const h=v62Health();
    state.tasks=Array.isArray(state.tasks)?state.tasks:[];
    state.tasks.unshift({id:v62Uid(),title:`15 минут: ${h.focus?h.focus.title:'разобрать главный завал дня'}`,area:h.focus?.area||'AI Coach',date:v62Today(),time:'',priority:'A',status:'Активно',google:false,reminder:'',note:'Создано AI Life Coach V62'});
    try{save()}catch(e){}
    v62Toast('Создал шаг на 15 минут');
    v62Go('tasks');
  }
  function v62CreateDebtTask(){
    const h=v62Health(); const d=h.dueOut[0]||h.dueIn[0];
    if(!d){v62Toast('Ближайших долгов не нашёл'); return v62Go('finance');}
    state.tasks=Array.isArray(state.tasks)?state.tasks:[];
    state.tasks.unshift({id:v62Uid(),title:`Финансы: ${d.direction==='out'?'оплатить/проверить':'напомнить о возврате'} ${d.person} · ${v62Money(d.amount)}`,area:'Финансы',date:v62Today(),time:'12:00',priority:'A',status:'Активно',google:false,reminder:'',note:`AI Coach V62. Срок: ${v62Fmt(d.due)}. ${d.note||''}`});
    try{save()}catch(e){}
    v62Toast('Финансовое напоминание создано');
    v62Go('tasks');
  }
  function v62CreateHabitTask(){
    const h=v62Health(); const habit=h.openHabits[0]||h.habits[0];
    state.tasks=Array.isArray(state.tasks)?state.tasks:[];
    state.tasks.unshift({id:v62Uid(),title:`Минимум ритма: ${habit?(habit.name||habit.title||'одна привычка'):'одна простая привычка'}`,area:'Привычки',date:v62Today(),time:'',priority:'B',status:'Активно',google:false,reminder:'',note:'Минимальная версия привычки, создано AI Life Coach V62'});
    try{save()}catch(e){}
    v62Toast('Задача для ритма создана');
    v62Go('tasks');
  }
  function v62CreateCoachBrief(){
    const h=v62Health();
    state.notes=Array.isArray(state.notes)?state.notes:[];
    state.notes.unshift({id:v62Uid(),title:'AI brief дня',folder:'Обзоры',date:v62Today(),text:v62BriefText(h)});
    state.settings.v62.lastBriefAt=new Date().toISOString();
    try{save()}catch(e){}
    v62Toast('AI brief создан в заметках');
    v62Go('notes');
  }
  function v62CreateMinimumDay(){
    const h=v62Health();
    state.tasks=Array.isArray(state.tasks)?state.tasks:[];
    const exists=new Set(state.tasks.filter(t=>t.date===v62Today()).map(t=>String(t.title||'').toLowerCase()));
    const adds=[
      `15 минут: ${h.focus?h.focus.title:'разобрать главный завал дня'}`,
      h.dueOut[0]?`Финансы: проверить ${h.dueOut[0].person} · ${v62Money(h.dueOut[0].amount)}`:'',
      h.openHabits[0]?`Минимум ритма: ${h.openHabits[0].name||h.openHabits[0].title||'одна привычка'}`:''
    ].filter(Boolean).filter(t=>!exists.has(t.toLowerCase()));
    adds.reverse().forEach((title,i)=>state.tasks.unshift({id:v62Uid(),title,area:i===1?'Финансы':'AI Coach',date:v62Today(),time:'',priority:i===0?'A':'B',status:'Активно',google:false,reminder:'',note:'Собрано AI Life Coach V62'}));
    try{save()}catch(e){}
    v62Toast(adds.length?`Собрал день: ${adds.length} задач`:'День уже собран');
    v62Go('daily');
  }
  function v62CopyBrief(){
    const text=v62BriefText(v62Health());
    if(navigator.clipboard) navigator.clipboard.writeText(text).then(()=>v62Toast('Brief скопирован')).catch(()=>v62Toast('Не удалось скопировать'));
    else v62Toast(text);
  }
  function v62SetMode(el){
    state.settings=state.settings||{}; state.settings.v62=state.settings.v62||{};
    state.settings.v62.mode=el.dataset.mode||'focus';
    try{save()}catch(e){}
    render();
  }

  const oldGoV62=typeof go==='function'?go:null;
  function v62Go(id){
    if(id==='coach'){
      page='coach';
      try{location.hash='coach'}catch(e){}
      try{if(typeof render==='function') render(); else v62Post();}catch(e){v62Post();}
      setTimeout(v62Post,80);
      return;
    }
    if(oldGoV62) return oldGoV62(id);
    try{location.hash=id; if(typeof render==='function') render();}catch(e){}
  }
  try{go=v62Go;}catch(e){}

  function v62Post(){
    try{
      v62Ensure(); v62Styles(); v62SetBuild();
      document.querySelectorAll('[data-v59-action="coach"]').forEach(el=>{el.removeAttribute('data-v59-action');el.setAttribute('data-go','coach');});
      document.querySelectorAll('[data-go="coach"]').forEach(el=>el.classList.add('v62-nav-hot'));
      const ai=document.querySelector('.v61-ai .v61-actions');
      if(ai && !ai.querySelector('[data-go="coach"]')) ai.insertAdjacentHTML('afterbegin','<button data-go="coach">AI Coach V62</button>');
      const current=(location.hash||'').replace('#','')||page||'dashboard';
      if(current==='coach'){
        const view=document.querySelector('#view');
        if(view) view.innerHTML=v62CoachPage();
        document.querySelectorAll('.v59-nav-item').forEach(el=>el.classList.toggle('active',el.dataset.go==='coach'));
      }
    }catch(e){console.error('[V62 post]',e)}
  }

  const oldRenderV62=typeof render==='function'?render:null;
  if(oldRenderV62){
    render=function(){
      v62Ensure();
      const r=oldRenderV62.apply(this,arguments);
      setTimeout(v62Post,90);
      return r;
    };
  }

  window.addEventListener('click',function(e){
    const a=e.target.closest&&e.target.closest('[data-v62-action]');
    if(!a) return;
    e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
    const act=a.dataset.v62Action;
    if(act==='setMode') return v62SetMode(a);
    if(act==='createTinyTask') return v62CreateTinyTask();
    if(act==='createDebtTask') return v62CreateDebtTask();
    if(act==='createHabitTask') return v62CreateHabitTask();
    if(act==='createCoachBrief') return v62CreateCoachBrief();
    if(act==='createMinimumDay') return v62CreateMinimumDay();
    if(act==='copyBrief') return v62CopyBrief();
  },true);
  window.addEventListener('hashchange',()=>{if((location.hash||'').replace('#','')==='coach') setTimeout(v62Post,90);});
...
(function(){
  ...
  function v63InjectPanel(pageId){
    ...
    const h=v63Health();
    let html='';
    if(pageId==='finance') html=`<section class="v63-card v63-injected"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><h3>Финансовый ориентир</h3><p>Лимит 7 дней: <b>${v63Money(h.safeLimit)}</b>. Ближайшие обязательства: <b>${v63Money(h.debtSum)}</b>. Прогноз месяца: <b>${v63Money(h.f.net||0)}</b>.</p></div><button class="v63-mini green" data-v63-action="createDebtPlan">Создать план</button></div></section>`;
    if(pageId==='tasks') html=`<section class="v63-card v63-injected"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><h3>Фокус выполнения</h3><p>${h.focus?`Главный шаг: ${v63Esc(h.focus.title)}.`:'Главный шаг не выбран.'} Просрочено: ${h.overdue.length}. Сегодня: ${h.todayTasks.length}.</p></div><button class="v63-mini blue" data-v63-action="createTinyTask">Шаг 15 минут</button></div></section>`;
    if(pageId==='calendar') html=`<section class="v63-card v63-injected"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><h3>Нагрузка календаря</h3><p>Нагрузка дня: <b>${h.score}%</b>. Учитываются задачи, просрочки, долги и привычки.</p></div><button class="v63-mini blue" data-go="command">Командный центр</button></div></section>`;
    if(pageId==='system') html=`<section class="v63-card v63-injected"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><h3>Безопасность данных</h3><p>Резервная копия, диагностика и контроль локальных данных.</p></div><button class="v63-mini blue" data-v63-action="backup">Скачать копию</button></div></section>`;
    if(html){const hero=view.querySelector('.v59-hero,.hero');if(hero)hero.insertAdjacentHTML('afterend',html);else view.insertAdjacentHTML('afterbegin',html)}
  }
  ...
  function v63Backup(){try{const payload={version:BUILD,exportedAt:new Date().toISOString(),state:exportableState()};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='second-brain-os-backup-'+v63Today()+'.json';document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},500);v63Toast('Backup скачан')}catch(e){console.error(e);v63Toast('Не удалось скачать backup')}}

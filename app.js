/* Second Brain OS — Operation Review Private V40.8 */
'use strict';

const RELEASE = 'v40-8-button-card-balance-private-20260629';
const DATA_VERSION = 408;
...
function render(){
  try{
    ...
    $('#app').innerHTML=shell();
    $('#releaseBadge').textContent='V40.8 BALANCE QA';
...
/* ===============================
   V40.8 BALANCE QA — final app pass
...
function shell(){
  const nav=navGroups.map(([title,items])=>`<div class="nav-group"><div class="nav-title">${title}</div>${items.map(([id,ic,label])=>`<button data-page="${id}" class="nav-btn ${activePage===id?'active':''}"><span class="nav-ic">${ic}</span><b>${label}</b></button>`).join('')}</div>`).join('');
  return `<div class="app shell-premium"><aside class="sidebar"><div class="brand"><div class="brand-mark">◔</div><div><b>Second Brain OS</b><span>Private Life RPG</span></div></div><nav class="nav">${nav}</nav><button class="quick-input" data-action="quickMenu"><span>⚡</span><b>Быстрый ввод</b><small>⌘ K</small></button><div class="sidebar-status v403-status"><small>PRIVATE APP</small><b>V40.8</b><span class="sync-dot">LIFE PLANS + AUTO RECONCILE</span><small>${new Date().toLocaleDateString('ru-RU')} ${new Date().toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</small></div></aside><main class="main"><header class="topbar premium-topbar"><div class="topbar-spacer"></div><div class="topbar-tools"><div class="search-box"><span>⌕</span><input id="search" class="search" placeholder="Поиск" value="${esc(searchQuery)}"></div><button class="ghost pwa-only-hide" data-action="installApp">Установить</button><button class="icon-btn" data-page="week" title="Календарь">◫</button><button class="icon-btn bell-btn" data-page="control" title="Уведомления"><span>◌</span><i>${attentionItems().length}</i></button><button class="avatar-btn" data-page="settings" title="Профиль">А</button></div></header><section class="mobile-page-title"><b>${mobileHeaderTitle()}</b><span>${new Date().toLocaleDateString('ru-RU',{day:'numeric',month:'long'})}</span></section><section class="view premium-view">${route()}</section></main>${mobileNav()}</div>`;
}
...
/* V40.8 OPERATIONS CLEANUP — safe period delete + CSV replace mode */
...
/* V40.8 OPERATION REVIEW — category + debt assignment center */
...
/* V40.8 RECONCILIATION + LIFE PLANNING — bank balance control + daily budget planning */
const SBOS_V406 = 'v40-8-button-card-balance-private-20260629';
...
importPage = function(){
  ...
  return __v406_oldImportPage().replace('Bank Import V40.8','Bank Import V40.8') + `<section class="card panel"><div class="section-head"><div><h3>Ежедневная сверка</h3><p class="sub">После импорта проверяй: операции банка ${r.count}, движение ${moneySigned(r.bankMovement)}, расходы в лимит ${money(r.limitExpense)}, на разбор ${r.uncat}.</p></div><button class="primary" data-page="reconcile">Открыть сверку</button></div></section>`;
};
...
render = function(){ __v406_oldRender(); const b=document.getElementById('releaseBadge'); if(b) b.textContent='V40.8 BALANCE QA'; };
...
/* V40.8 BUTTON QA + CARD BALANCE — modal click fix and manual card/account facts */
const SBOS_V408 = 'v40-8-button-card-balance-private-20260629';
...
render = function(){ __v408_oldRender(); const b=document.getElementById('releaseBadge'); if(b) b.textContent='V40.8 BALANCE QA'; };

/* Second Brain OS — Operation Review Private V40.6 */
'use strict';

const RELEASE = 'v40-6-reconciliation-planning-private-20260629';
...
function render(){
  try{
    ...
    $('#app').innerHTML=shell();
    $('#releaseBadge').textContent='V40.6 RECONCILE';
...
/* ===============================
   V40.6 RECONCILE — final app pass
...
/* V40.6 OPERATIONS CLEANUP — safe period delete + CSV replace mode */
...
/* V40.6 OPERATION REVIEW — category + debt assignment center */
...
/* V40.6 RECONCILIATION + LIFE PLANNING — bank balance control + daily budget planning */
const SBOS_V406 = 'v40-6-reconciliation-planning-private-20260629';
...
confirmBankImport = function(){
  ...
  const batchId=logBankImport('new-only', pendingBankImport, range, {fileName: pendingBankImportMeta?.fileName || 'bank.csv', duplicates: pendingBankImportAll.filter(x=>x.duplicate).length});
...
confirmBankReplaceImport = function(){
  ...
  const deleted=before-state.operations.length;
  const imported=cleanImportedBankRows(pendingBankImportAll);
  const batchId=logBankImport('replace-period', imported, range, {fileName: pendingBankImportMeta?.fileName || 'bank.csv', deleted});
...
function createOperationFromLifePlan(p){
  const op={id:uid(), date:p.date||todayKey(), type:'expense', amount:num(p.amount), category:p.category||'Личная жизнь', note:`План: ${p.title||'личная жизнь'}`, source:'life-plan', sourceId:'life_'+p.id, lifePlanId:p.id, excludeFromLimit:p.includeInBudget===false, includeInBalance:false};
...
importPage = function(){
  ...
  return __v406_oldImportPage().replace('Bank Import V40.6','Bank Import V40.6') + `<section class="card panel"><div class="section-head"><div><h3>Ежедневная сверка</h3><p class="sub">После импорта проверяй: операции банка ${r.count}, движение ${moneySigned(r.bankMovement)}, расходы в лимит ${money(r.limitExpense)}, на разбор ${r.uncat}.</p></div><button class="primary" data-page="reconcile">Открыть сверку</button></div></section>`;
};
...
try{
  ...
  const dayGroup=navGroups.find(g=>g[0]==='День');
  if(dayGroup && !dayGroup[1].some(x=>x[0]==='planning')) dayGroup[1].splice(2,0,['planning','◇','Планирование']);

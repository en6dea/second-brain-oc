...
/**
 * Открываем без явной версии: базу мог создать другой код, и навязывание
 * версии сломало бы его.
 *
 * Если хранилища нет (чистое устройство), создаём его **версией 1** — ровно
 * такой, какую ожидает первая версия приложения. Поднимать номер выше нельзя:
 * первая версия открывала базу с номером 1 и при более высоком получала
 * VersionError, видела пустое состояние и записывала пустоту поверх.
 */
async function openDb() {
  ...
  db.close();
  /* Свежесозданная база пуста и уже имеет версию 1 — просим ту же, чтобы
     сработал onupgradeneeded и появилось хранилище. */
  db = await openRaw(current <= 1 ? 1 : current + 1);

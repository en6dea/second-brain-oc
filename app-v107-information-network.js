...
'use strict';
(() => {
  const BUILD = window.SecondBrainBuild || { id: 'second-brain-os-v107-connected-information-20260731-r1', label: 'V107 · CONNECTED INFORMATION' };
  ...
  const ENTITY = {
    people: {collection:'people', label:'Человек', icon:'👥', title:item=>item?.name||'Без имени', route:item=>`person-${item.id}`},
    notes: {collection:'notes', label:'Заметка', icon:'📝', title:item=>item?.title||'Без названия', route:item=>`note-${item.id}`},
    ideas: {collection:'ideas', label:'Идея', icon:'💡', title:item=>item?.title||'Без названия', route:item=>`idea-${item.id}`},
    goals: {collection:'goals', label:'Цель', icon:'🎯', title:item=>item?.title||'Цель', route:item=>`goal-${item.id}`},
    tasks: {collection:'tasks', label:'Задача', icon:'✓', title:item=>item?.title||'Задача', route:()=>`tasks`},
    plans: {collection:'lifePlans', label:'План', icon:'🧭', title:item=>item?.title||'План', route:item=>`plan-${item.id}`},
    learning: {collection:'learningMaterials', label:'Обучение', icon:'🎓', title:item=>item?.title||'Материал', route:item=>`learning-item-${item.id}`},
    personal: {collection:'personal', label:'Воспоминание', icon:'◇', title:item=>item?.title||'Воспоминание', route:item=>`memory-${item.id}`},
    trips: {collection:'trips', label:'Поездка', icon:'✈️', title:item=>item?.title||item?.name||'Поездка', route:()=>`trips`},
    documents: {collection:'documents', label:'Документ', icon:'📄', title:item=>item?.title||item?.name||'Документ', route:()=>`documents`}
  };
  ...
  function ensureData() {
    ...
    state.settings.v107=Object.assign({version:1,memoryView:'timeline',polinaPrivate:true,hidePolinaFromHome:false,excludePolinaCloud:false,confirmPolinaExport:true},state.settings.v107||{});
    ...
    state.lifePlans=arr(state,'lifePlans').filter(Boolean).map(item=>normalizeItem(item,{title:'',type:'blank',why:'',outcome:'',due:'',status:'planned',priority:'medium',budget:0,actualSpend:0,participants:[],documents:[],externalLinks:[],notes:'',risks:'',nextAction:'',blocks:[],archivedBlocks:[]}));
  ...
  function informationPage() {
    ...
    const folders=[
      ...
      ['trips','✈️','Путешествия','Поездки, бронирования и маршруты',arr(state,'trips').length],
    ...
    return `<section class="v78-page v107-page"><header class="v78-page-head v107-page-head"><div><span>Связанная база знаний</span><h1>Информация</h1><p>Сохраняйте контекст, связывайте записи только когда это полезно и превращайте знания в действия.</p></div><div class="v107-head-actions"><button class="v78-secondary" data-v107-action="open-global-search" type="button">⌕ Найти</button><button class="v78-primary" data-v107-action="open-quick-create" type="button">＋ Добавить</button></div></header>
      <section class="v107-info-grid">${folders.map(([route,icon,title,text,count])=>`<button class="v107-folder" data-v107-info-route="${route}" type="button"><i>${icon}</i><span><b>${esc(title)}</b><small>${esc(text)}</small></span><em>${count||'—'}</em><u>›</u></button>`).join('')}</section>
  ...
  function planDetail(id){
    ...
    const checklist=blocks.flatMap(block=>Array.isArray(block.items)?block.items:[]).filter(item=>item&&typeof item==='object'&&'done'in item);
    ...
    return `<section class="v78-page v107-page"><header class="v107-detail-head"><div><button class="v78-back" data-v107-info-route="planner">← Планер</button><small>${planTypeLabel(plan.type)} · ${planStatusLabel(plan.status)}</small><h1>${esc(plan.title)}</h1><p>${esc(plan.why||plan.outcome||'')}</p></div><div class="v107-head-actions"><button class="v78-secondary" data-v107-action="edit-plan" data-id="${id}">Редактировать</button><button class="v78-primary" data-v107-action="add-plan-block" data-id="${id}">＋ Добавить блок</button></div></header>
      <section class="v107-plan-summary"><article><small>Срок</small><b>${formatDate(plan.due)}</b></article><article><small>Бюджет</small><b>${plan.budget?money(plan.budget):'—'}</b></article><article><small>Факт</small><b>${plan.actualSpend?money(plan.actualSpend):'—'}</b></article><article><small>Чек-листы</small><b>${progress}%</b></article><article class="wide"><small>Следующий шаг</small><b>${esc(plan.nextAction||'Не определён')}</b></article></section>
      <section class="v107-detail-grid"><div class="v107-plan-blocks">${blocks.map((block,index)=>planBlock(plan,block,index)).join('')||empty('Добавьте только те блоки, которые нужны для этой ситуации.')}${plan.archivedBlocks?.length?`<button class="v107-restore" data-v107-action="restore-plan-block" data-id="${id}">↶ Восстановить последний удалённый блок</button>`:''}</div><aside>${relationsPanel('plans',id)}${smartAssistant(plan.nextAction?'Следующий шаг определён':'Сделайте план исполнимым',plan.nextAction?plan.nextAction:'Добавьте одно физическое действие: открыть, проверить, позвонить, купить или забронировать.',[{action:'create-plan-task',label:'Создать задачу',data:`data-id="${id}"`}])}<button class="v107-danger" data-v107-action="delete-entity" data-type="plans" data-id="${id}">Удалить план</button></aside></section>
    </section>`;
  }
  ...
  function polinaEnhance(view){
    ...
    if(hero){
      ...
      summary.innerHTML=`<article><small>Энергия</small><b>${latest.energy?`${latest.energy}/10`:'—'}</b></article><article><small>Дискомфорт</small><b>${latest.discomfort?`${latest.discomfort}/10`:'—'}</b></article><article><small>Сон</small><b>${esc(latest.sleep||'—')}</b></article><article class="wide"><small>Какая поддержка нужна</small><b>${esc(latest.supportNeeded||latest.request||'Пока не указано')}</b></article>`;
  ...
  function openQuickCreate(){
    openModal('Что добавить?',`<div class="v107-quick-grid">${[['edit-person','👥','Человека'],['edit-note','📝','Заметку'],['edit-idea','💡','Идею'],['edit-memory','◇','Воспоминание'],['edit-learning','🎓','Материал обучения'],['edit-plan','🧭','План']].map(([action,icon,label])=>`<button data-v107-action="${action}"><i>${icon}</i><b>${label}</b></button>`).join('')}</div><div class="v107-modal-actions"><button data-v107-action="close-modal">Отмена</button></div>`);
  }
  ...
  function updateGlobalSearch(query){
    ...
    host.innerHTML=results.slice(0,80).map(({type,item,title})=>`<button data-v107-info-route="${esc(entityRoute(type,item.id))}"><i>${ENTITY[type].icon}</i><span><small>${esc(ENTITY[type].label)} · ${linkedCount(type,item.id)} связей</small><b>${esc(title)}</b><p>${esc(searchSnippet(item,q))}</p></span><em>›</em></button>`).join('')||empty('Ничего не найдено.');
  }
  ...
  async function boot(){
    ...
    if(needsMigration){
      try{
        const backupAttempt=Promise.resolve(window.SecondBrainBackup?.create?.({reason:'automatic-before-v107-information-migration'})).catch(error=>console.warn('[V107 backup]',error));
